...
description: |
  ...
  Do NOT use this agent for: simple `/navgator:scan` requests (use the scan command directly), one-shot file reads, or non-architectural questions. Do NOT auto-trigger a full scan; if state demands one, return that as a recommendation for the user to confirm.
model: opus
color: magenta
...
1. **Does `.navgator/architecture/index.json` exist?**
   ...
   ```
   - MISSING → recommend `/navgator:scan` (full first scan). Stop. Do NOT run a scan yourself.
...
4. **Run incremental scan when freshness gate fires.**
   ...
   - If `--incremental` ran but had `no-prior-state` (first-ever scan), abort the agent flow and recommend `/navgator:scan`.
...
- **You MAY run `navgator scan --auto`; use explicit incremental mode only when the user asks to override auto selection.**
