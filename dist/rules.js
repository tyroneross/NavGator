...
import { classifyPathConvention, detectEntryPoints, entryCandidatePaths, } from './entry-points.js';
 ...
 */
const ORPHAN_EXEMPT_ENTRY_SOURCES = new Set([
    'package-entry',
    'package-script',
    'package-file',
    'framework-route',
 ...
 */
function isFileNode(c) {
    if (c.tags?.includes('module') || c.tags?.includes('file-node'))
        return true;
    return c.metadata?.['kind'] === 'source-file';
}
/**
 ...
 */
export function getBuiltinRules(projectRoot) {
    return [
        {
            id: 'orphan-component',
            ...
            check: (components, connections) => {
                ...
                return unconnected
                    ...
                    if (source && ORPHAN_EXEMPT_ENTRY_SOURCES.has(source))
                        return false;
        ...
        },
        {
            ...
            check: (components, connections) => {
                ...
                const backendComponents = new Set(components.filter(c => c.role.layer === 'backend').map(c => c.component_id));
                return dbComponents
                    .filter(db => {
                    const incomingFromBackend = connections.some(conn => conn.to.component_id === db.component_id && backendComponents.has(conn.from.component_id));
 ...
 */
function checkDuplicateResourceCreation(components, connections) {
    const violations = [];
    // Find service and llm components
    const serviceComponents = components.filter(c => c.type === 'service' || c.type === 'llm');
    for (const svc of serviceComponents) {
        // Find all connections TO this service (i.e., components that use/import it)
        const importers = connections.filter(c => c.to.component_id === svc.component_id);
        if (importers.length <= 1)
            continue;
        // Check code snippets for instantiation patterns: ClassName()
        const instantiationSites = [];
 ...
 */
const DEAD_CODE_EXCLUDED_TYPES = new Set([
    'npm',
    'pip',
    'spm',
    'cargo',
...
function analyzeTransitiveDeadCode(components, connections, projectRoot) {
    ...
    for (const conn of connections) {
        ...
        if (conn.connection_type === 'target-contains')
            continue;
    ...
    const externalNames = new Set(components.filter(c => EXTERNAL_PACKAGE_TYPES.includes(c.type)).map(c => c.name));
    // Components not reachable and not excluded types are transitively dead
    const violations = [];
    for (const c of components) {
        if (reachable.has(c.component_id))
            continue;
        if (DEAD_CODE_EXCLUDED_TYPES.has(c.type))
            continue;
 ...
 */
export function detectRuleDegeneracy(counts, population, threshold = RULE_DEGENERACY_SHARE) {
    ...
    return {
        ...
        warnings: degenerate.map(d => `Rule '${d.rule_id}' fires on ${d.components} of ${population} components ` +
 ...
 */
export function countComponentsPerRule(violations) {
    const seen = new Map();
    ...
    const counts = {};
    for (const [rule_id, set] of seen)
        counts[rule_id] = set.size;
    return counts;
}
/**
 * Load custom rules from .navgator/architecture/rules.json
 */
export function loadCustomRules(projectRoot) {
    const root = projectRoot || process.cwd();
    ...
    try {
        ...
        if (!Array.isArray(parsed))
            return [];
        return parsed.map(convertCustomRule).filter((r) => r !== null);
    }
    catch {
        return [];
    }
}
function convertCustomRule(json) {
    if (!json.id || !json.severity)
        return null;
    return {
        id: json.id,
...
function matchesPattern(component, pattern) {
    if (!pattern)
        return true; // No pattern = matches everything
    if (pattern.layer && component.role.layer !== pattern.layer)
        return false;
    if (pattern.type && component.type !== pattern.type)
        return false;
 ...
 */
export function checkRules(components, connections, rules, projectRoot) {
 ...
 */
export function formatRulesOutput(violations, filterSeverity) {
    ...
    const severityIcons = { error: 'ERROR', warning: 'WARN', info: 'INFO' };
