...
export function discoverStackRoots(root, verbose) {
    ...
    for (const origin of sortedOrigins) {
        if (origin !== '.' && retained.some(parent => parent !== '.' && (origin === parent || origin.startsWith(`${parent}/`))))
            continue;
 ...
 */
function normalizeForcedChangedFiles(files, root) {
...
export function selectScanMode(fileChanges, index, options, now = Date.now()) {
    ...
    const changedMatchesTrigger = (file, trigger) => file === trigger || file.endsWith(`/${trigger}`);
 ...
 */
export function isDiffConsistentWithScan(entry, finalComponentCount) {
    if (!entry?.diff)
        return false;
...
export async function scan(projectRoot, options = {}) {
    ...
    let degraded;
    {
        ...
        if (disabledCapabilities.length > 0) {
            ...
            if (restrictions.includes('readOnlyFs')) {
                messageParts.push('The environment also reports a read-only filesystem (readOnlyFs); no additional capability was disabled for it because Phase-3 analysis performs no filesystem writes.');
            }
            degraded = {
                restrictions,
    ...
    const acquisition = options._scanLease
        ? { ok: true, lease: options._scanLease }
    ...
    try {
        ...
        const forcedChangedFiles = normalizeForcedChangedFiles(options._forcedChangedFiles ?? [], root);
        for (const forced of forcedChangedFiles) {
            if (!fileChanges.added.includes(forced) &&
                !fileChanges.modified.includes(forced) &&
                !fileChanges.removed.includes(forced)) {
                if (fs.existsSync(path.join(root, forced)))
                    fileChanges.modified.push(forced);
                else
                    fileChanges.removed.push(forced);
            ...
            if (swiftStackRoots.length > 0) {
                ...
                try {
                    ...
                    const swiftKnownPackages = allComponents
                        .filter(c => (c.tags?.includes('swift') && c.tags?.includes('import')) ||
            ...
            if (stackRoots.some(sr => detectCargo(sr.path))) {
                ...
                try {
                    ...
                    const cargoKnownPackages = allComponents
                        ...
                        .map(c => {
                        const crateName = c.metadata?.['crate_name'];
                        return {
        ...
        if (decision.mode === 'incremental' && !options.clearFirst) {
            ...
            const partition = partitionPriorStateForFiles(preClearComponents, preClearConnections, walkSet, root);
