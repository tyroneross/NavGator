 ...
 */
export function traceDataflow(startComponent, allComponents, allConnections, options = {}) {
    ...
    if (resolveFileNodes) {
        for (const comp of allComponents) {
            for (const f of comp.source.config_files || []) {
                const fileId = `FILE:${f}`;
                if (!fileOwnerMap.has(fileId))
                    fileOwnerMap.set(fileId, comp);
            }
        }
    }
    const paths = [];
    ...
    for (const conn of allConnections) {
        if (!outgoing.has(conn.from.component_id))
            outgoing.set(conn.from.component_id, []);
        outgoing.get(conn.from.component_id).push(conn);
        if (!incoming.has(conn.to.component_id))
            incoming.set(conn.to.component_id, []);
    ...
    for (const comp of allComponents) {
        for (const f of comp.source.config_files || []) {
            ...
            if (fileOut) {
                if (!outgoing.has(comp.component_id))
                    outgoing.set(comp.component_id, []);
 ...
 */
export function formatTraceOutput(result) {
    const lines = [];
