...
export function registryLockPath(dir) {
 ...
 */
async function tryCreate(lockPath, record) {
    let handle;
    try {
 ...
 * losing an entry.
 */
async function stealIfStale(lockPath, staleMs) {
    let age = null;
    let observed = null;
    try {
    ...
    try {
        ...
        if (observed !== null && confirm !== observed)
            return;
    }
    catch {
 ...
 * problems.
 */
export async function acquireRegistryLock(dir, options = {}) {
 ...
 */
export async function withRegistryFileLock(dir, fn, options = {}) {
...
//# sourceMappingURL=registry-lock.js.map