 ...
 */
export async function buildExploreReport(query, opts = {}) {
    ...
    const incoming = incomingConns.map(c => {
 ...
 */
export function formatExploreReport(report) {
    const lines = [
    ...
    if (report.runtime) {
        ...
        if (r.host)
            parts.push(`host: ${r.host}${r.port ? `:${r.port}` : ""}`);
    ...
    if (report.outgoing.length > 0) {
        ...
        if (report.outgoing.length > 10)
            lines.push(`  ... +${report.outgoing.length - 10} more`);
    }
    if (report.incoming.length > 0) {
        ...
        if (report.incoming.length > 10)
            lines.push(`  ... +${report.incoming.length - 10} more`);
    }
    if (report.trace.paths.length > 0) {
        ...
        if (report.trace.paths.length > 5)
            lines.push(`  ... +${report.trace.paths.length - 5} more paths`);
    }
    return lines.join("\n");
}
//# sourceMappingURL=explore-report.js.map