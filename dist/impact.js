 ...
 * - low: 0-1 dependents
 */
export function computeSeverity(component, dependentCount) {
 ...
 */
export function computeImpact(component, allComponents, allConnections) {
    // Find direct dependents — connections TO this component (things that use it)
    const directConnections = allConnections.filter((c) => c.to.component_id === component.component_id);
    const affected = [];
    const affectedFiles = new Set();
    const directComponentIds = new Set();
    // Direct dependents
    for (const conn of directConnections) {
        const dependentComponent = allComponents.find((c) => c.component_id === conn.from.component_id);
    ...
    for (const directId of directComponentIds) {
        const transitiveConnections = allConnections.filter((c) => c.to.component_id === directId && !directComponentIds.has(c.from.component_id) && c.from.component_id !== component.component_id);
        for (const conn of transitiveConnections) {
            const transitiveComponent = allComponents.find((c) => c.component_id === conn.from.component_id);
            if (!transitiveComponent)
                continue;
