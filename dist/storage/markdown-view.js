...
function escapeYaml(value) {
    ...
    if (/^[\w./:_-]+$/.test(value) && value.length < 80)
        return value;
...
function buildFrontmatter(c, metricLookup) {
    ...
    if (c.runtime?.engine)
        fields.runtime_engine = c.runtime.engine;
    if (c.runtime?.resource_type)
        fields.runtime_type = c.runtime.resource_type;
    if (c.runtime?.connection_env_var)
        fields.runtime_env = c.runtime.connection_env_var;
    if (c.documentation_url)
        fields.documentation_url = c.documentation_url;
    const lines = ['---'];
...
function buildBody(c, neighbors) {
    const lines = [];
    lines.push(`# ${c.name}`);
    lines.push('');
    if (c.role?.purpose) {
    ...
    if (neighbors.outgoing.length || neighbors.incoming.length) {
        ...
        if (neighbors.outgoing.length) {
            lines.push('### Outgoing');
            for (const id of neighbors.outgoing)
                lines.push(`- [[${id}]]`);
        }
        if (neighbors.incoming.length) {
            lines.push('### Incoming');
            for (const id of neighbors.incoming)
                lines.push(`- [[${id}]]`);
        }
        lines.push('');
    }
 ...
 */
export async function writeComponentMarkdownViews(storeDir, components, connections) {
    ...
    const idOf = (c) => c.stable_id ?? generateStableId(c.type, c.name);
    const componentById = new Map();
    for (const c of components)
        componentById.set(c.component_id, c);
    const neighbors = new Map();
    for (const c of components)
        neighbors.set(c.component_id, { incoming: [], outgoing: [] });
    ...
    if (fs.existsSync(metricsPath)) {
        try {
            ...
            if (!report.suppressed) {
                metricLookup = new Map(report.metrics.map((m) => [m.component_id, { pagerank_score: m.pagerank_score, community_id: m.community_id }]));
            }
        }
        catch {
    ...
    for (const c of components) {
        ...
        const content = buildFrontmatter(c, metricLookup) +
            '\n\n' +
 ...
 */
export async function writeConnectionsJsonl(storeDir, components, connections) {
    const filePath = path.join(storeDir, CONNECTIONS_JSONL);
    const idOf = (c) => c ? (c.stable_id ?? generateStableId(c.type, c.name)) : undefined;
