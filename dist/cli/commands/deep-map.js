...
import { generateRunId, isValidRunId, listPacketFiles, readFindings, readIngestReport, readLatestRunId, readManifest, writeLatest, writeManifest, writePacket, } from '../../deep-map/store.js';
import { DEEP_MAP_LIMITS, DEEP_MAP_SCHEMA_VERSION, } from '../../deep-map/types.js';
...
function parseIntOption(value, opts = {}) {
    if (value === undefined)
        return null;
...
function parseFloatOption(value, opts = {}) {
    if (value === undefined)
        return null;
    const trimmed = value.trim();
    if (!/^-?\d+(\.\d+)?$/.test(trimmed))
...
function resultPathForPacketFile(packetFile) {
    return packetFile.replace(/\.json$/, '.result.json');
}
// =============================================================================
// REGISTRATION
// =============================================================================
export function registerDeepMapCommand(program) {
...
async function runPlan(options) {
    ...
    const maxPackets = options.maxPackets !== undefined
    ...
    const maxDeep = options.maxDeep !== undefined ? parseIntOption(options.maxDeep, { min: 0 }) : DEEP_MAP_LIMITS.maxDeep;
    if (maxDeep === null) {
        failUsage(`invalid --max-deep value: ${JSON.stringify(options.maxDeep)}`);
        return;
    }
    const minGroup = options.minGroup !== undefined ? parseIntOption(options.minGroup, { min: 1 }) : DEEP_MAP_LIMITS.minGroup;
    if (minGroup === null) {
    ...
    const escalateThreshold = options.escalateThreshold !== undefined
    ...
    const partition = partitionComponents(tier0.components, tier0.connections, tier0.metrics, {
        ...filterOptions,
        minGroup,
    ...
    const communities = !tier0.metrics || tier0.metrics.suppressed
        ? 0
    ...
    if (continuingRun && existingManifest) {
        ...
        manifest.cost = {
            ...
            estimated_input_tokens: manifest.packets.reduce((sum, p) => sum + p.estimated_input_tokens, 0),
        };
        if (!escalation && existingManifest.escalation)
            manifest.escalation = existingManifest.escalation;
    }
...
function printPlanHuman(runId, packets, partition, escalation, totalEstimatedTokens) {
    ...
    for (const p of packets) {
        ...
        console.log(`  ${p.packet_id}  ${p.group_label}  ${p.components.length} components  ~${p.estimated_input_tokens} tokens  ${prefix}`);
    }
    if (escalation) {
        console.log('');
    ...
    console.log('');
    console.log(`Filter: excluded_vendor=${partition.filter.excluded_vendor}  excluded_glob=${partition.filter.excluded_glob}  ` +
        `suspect_vendored=${partition.filter.suspect_vendored}`);
    console.log('');
    console.log(`Total estimated input tokens: ${totalEstimatedTokens}`);
    console.log('');
    console.log(`Next: fan each packet out to a subagent, write its result to <packet_id>.result.json, then run ` +
        `\`navgator deep-map ingest --run ${runId}\`.`);
}
// =============================================================================
// ingest
// =============================================================================
function registerIngest(deepMap) {
    deepMap
        .command('ingest')
...
function printIngestHuman(report) {
...
// report
// =============================================================================
function registerReport(deepMap) {
    deepMap
        .command('report')
...
async function runReport(options) {
    ...
    if (report.findings_by_component.length === 0 && report.cross_cutting.length === 0) {
        failNoData(`Run ${runId} has no findings yet — run \`navgator deep-map ingest --run ${runId}\` first.`);
        return;
    }
    if (options.agent) {
...
// status
// =============================================================================
function registerStatus(deepMap) {
    deepMap
        .command('status')
...
async function runStatus(options) {
    ...
    console.log(`Packets: ${payload.packets_with_results}/${payload.packets_planned} have results ` +
