...
import { checkRules, countComponentsPerRule, describeReachability, detectRuleDegeneracy, getBuiltinRules, loadCustomRules, formatRulesOutput, } from '../../rules.js';
 ...
 */
export function measureRuleDegeneracy(violations, componentCount) {
...
export function formatReachabilityWarnings(d) {
    ...
    if (d.suppressed.vendored > 0) {
        lines.push(`${d.suppressed.vendored} component(s) skipped as vendored third-party code. ` +
            `A first-party directory under packages/ whose name matches a dependency is ` +
            `skipped by the same test.`);
    }
    return lines;
}
export function buildRulesAgentData(violations, rulesChecked, severity, degeneracy, reachability) {
...
export function registerRulesCommand(program) {
    program
        ...
        try {
            ...
            if (options.agent) {
                const data = buildRulesAgentData(violations, allRules.length, options.severity, degeneracy, reachability);
