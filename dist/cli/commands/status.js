    ...
    program
        ...
        try {
            ...
            if (index.project?.type && index.project.type !== 'unknown') {
                const stack = [index.project.type];
                if (index.project.platforms?.length)
                    stack.push(index.project.platforms.join('/'));
                if (index.project.architecture_pattern)
                    stack.push(index.project.architecture_pattern);
            ...
            if (Object.keys(index.stats.connections_by_type).length > 0) {
                ...
                for (const s of archBreakdown)
                    console.log(`  ${s}`);
                if (codeCount > 0) {
                    console.log(`CODE CONNECTIONS (${codeCount}):`);
                    for (const s of codeBreakdown)
                        console.log(`  ${s}`);
                }
            }
            ...
            if (Object.keys(infraCounts).length > 0) {
                ...
                try {
                    ...
                    if (summaryComp?.metadata?.report) {
                        const r = summaryComp.metadata.report;
                        console.log(`  Field usage: ${r.totalFields} fields across ${r.scannedModels} models`);
                        if (r.unusedFields > 0)
                            console.log(`    Unused fields: ${r.unusedFields} (run 'navgator coverage --fields' for details)`);
                        if (r.writeOnlyFields > 0)
                            console.log(`    Write-only fields: ${r.writeOnlyFields}`);
                    }
                }
                catch {
            ...
            try {
                ...
                const orphans = allComps.filter(c => orphanTypes.has(c.type) &&
            ...
            try {
                ...
                for (const q of qComps) {
                    const consumers = qConns.filter(c => c.from.component_id === q.component_id && c.connection_type === 'queue-consumes');
            ...
            try {
                ...
                if (timeline && timeline.entries.length > 0) {
                    ...
                    if (diff.components.added.length > 0)
                        parts.push(`+${diff.components.added.length} components`);
                    if (diff.components.removed.length > 0)
                        parts.push(`-${diff.components.removed.length} components`);
                    if (diff.connections.added.length > 0)
                        parts.push(`+${diff.connections.added.length} connections`);
                    if (diff.connections.removed.length > 0)
                        parts.push(`-${diff.connections.removed.length} connections`);
                    if (parts.length > 0) {
