...
function emit(command, data, options) {
...
function formatLessonSummary(l) {
    const tags = l.applies_to;
    const source = l.source_project;
    const promotedMarker = l.promoted ? ' [promoted]' : '';
...
function printTable(title, lessons) {
    console.log(title);
    ...
    console.log(`  ${'ID'.padEnd(40)} ${'SEVERITY'.padEnd(9)} ${'CATEGORY'.padEnd(24)} PATTERN`);
    for (const l of lessons)
        console.log(formatLessonSummary(l));
}
function printLessonDetail(l) {
    console.log(`ID:        ${l.id}`);
    ...
    const g = l;
    if (g.source_project)
        console.log(`Source:    ${g.source_project}`);
    if (g.applies_to && g.applies_to.length > 0) {
        console.log(`Tags:      ${g.applies_to.join(', ')}`);
    }
    if (g.promoted_at)
        console.log(`Promoted:  ${g.promoted_at}`);
    if (g.promoted_from)
        console.log(`From:      ${g.promoted_from}`);
    if (l.promoted)
        console.log(`Promoted:  true (local)`);
    if (l.context) {
        console.log('Context:');
        if (l.context.first_seen)
            console.log(`  first_seen: ${l.context.first_seen}`);
        if (l.context.last_seen)
            console.log(`  last_seen:  ${l.context.last_seen}`);
        ...
        if (l.context.resolution)
            console.log(`  resolution: ${l.context.resolution}`);
    }
    if (l.example) {
        console.log('Example:');
        if (l.example.bad)
            console.log(`  bad:  ${l.example.bad}`);
        if (l.example.good)
            console.log(`  good: ${l.example.good}`);
        if (l.example.why)
            console.log(`  why:  ${l.example.why}`);
    }
}
// =============================================================================
// COMMAND
// =============================================================================
export function registerLessonsCommand(program) {
    ...
    lessons
        ...
        try {
            ...
            let data = [];
            if (options.all) {
                const local = listLessons({ scope: 'local', projectRoot });
                const global = listLessons({ scope: 'global' });
                data = [...local, ...global];
    ...
    lessons
        ...
        .option('--tag <tag>', 'Filter by applies_to tag (repeatable, global only)', collect, [])
        .option('--json', 'Output as JSON')
        .option('--agent', 'Output wrapped in agent envelope (implies --json)')
    ...
    lessons
        ...
        .option('--tag <tag>', 'applies_to tag (repeatable)', collect, [])
