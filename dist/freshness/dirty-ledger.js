...
import { dirtyEventsPath, dirtyLedgerPath, dirtyMutationLockPath, stampPath, } from './paths.js';
...
function writeEvent(paths, root) {
    ...
    const payload = { version: 2, paths: values, created_at: Date.now() };
    try {
...
function readLegacyFile(filePath, afterRead) {
    let fd;
    try {
...
function readLegacy(root, afterRead) {
 ...
 */
export function withDirtyLedgerMutationLock(root, action) {
...
function tryWithDirtyLedgerMutationLock(root, action) {
...
function refreshStampAfterDirtyMutation(root, hooks = {}) {
...
export function captureDirtySnapshot(root, hooks = {}) {
    ...
    for (const file of files) {
        ...
        if (event)
            events.push({ file, paths: event.paths });
    }
    return {
...
export function markDirty(paths, root, hooks = {}) {
    if (normalizedPaths(paths).length === 0)
        return;
    ...
    if (!fs.existsSync(stampPath(root)))
        return;
    try {
...
function clearCapturedLegacy(snapshot, root) {
    if (!snapshot.legacyFingerprint)
        return;
...
export function clearDirtySnapshot(snapshot, root, hooks = {}) {
    hooks.beforeSnapshotDelete?.();
    withDirtyLedgerMutationLock(root, () => {
        const dir = dirtyEventsPath(root);
        for (const event of snapshot.events) {
            try {
                fs.unlinkSync(path.join(dir, event.file));
            }
