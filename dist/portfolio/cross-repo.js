...
export function buildCrossRepoMap(repos) {
    return {
 ...
 */
function findCrossRepoServiceCalls(repos) {
    const edges = [];
    ...
    for (const repoA of repos) {
        ...
        for (const conn of repoA.connections) {
            if (!CONNECTION_TYPES.has(conn.connection_type))
                continue;
    ...
    return edges.sort((a, b) => a.fromRepo.localeCompare(b.fromRepo) ||
...
function buildStatus(repos) {
    ...
    for (const r of repos) {
        ...
        if (r.lastScan && now - r.lastScan > STALE_MS)
            staleRepos.push(r.repo);
        if (r.scanStatus === 'failed')
            failedRepos.push(r.repo);
        if (r.scanStatus === 'busy')
            busyRepos.push(r.repo);
    }
    return {
