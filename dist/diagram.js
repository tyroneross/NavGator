...
export function generateMermaidDiagram(graph, options = {}) {
    ...
    const sortedLayers = Array.from(nodesByLayer.keys()).sort((a, b) => LAYER_STYLES[a].order - LAYER_STYLES[b].order);
 ...
 */
export function generateComponentDiagram(graph, componentId, depth = 1, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    const lines = [];
    ...
    const connectedNodeIds = new Set([componentId]);
    const relevantEdges = [];
 ...
 */
export function generateLayerDiagram(graph, layer, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, direction: 'LR', ...options };
    ...
    const relevantEdges = graph.edges.filter(e => layerNodeIds.has(e.source) || layerNodeIds.has(e.target));
 ...
 */
export function generateSummaryDiagram(graph, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
 ...
 */
export function wrapInMarkdown(diagram, title) {
    const lines = [];
    if (title) {
        lines.push(`## ${title}`);
        lines.push('');
