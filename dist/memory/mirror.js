...
import { readProjectMemory, listProjectMemories } from './store.js';
 ...
 */
function scanMirroredProjects(targetRoot) {
    let count = 0;
    let lastMirroredAt = null;
    let entries;
    try {
    ...
    for (const entry of entries) {
        if (!entry.isDirectory())
            continue;
        const jsonPath = path.join(targetRoot, PROJECTS_DIRNAME, entry.name, ARCHITECTURE_DIRNAME, JSON_FILENAME);
        try {
            const stat = fs.statSync(jsonPath);
            count += 1;
            if (lastMirroredAt === null || stat.mtimeMs > lastMirroredAt)
                lastMirroredAt = stat.mtimeMs;
        }
        catch {
 ...
 */
function resolveTargetSlug(targetRoot, baseNameSlug, resolvedProjectPath, gatorMemorySlug) {
    const candidateJson = path.join(targetRoot, PROJECTS_DIRNAME, baseNameSlug, ARCHITECTURE_DIRNAME, JSON_FILENAME);
    const existing = readJsonSafe(candidateJson);
...
// RENDERING
// =============================================================================
function formatMilestoneLine(event) {
    const date = new Date(event.ts).toISOString().slice(0, 10);
    const detail = event.detail;
    if (event.kind === 'architecture.changed' && detail) {
        const parts = [];
        if (detail.componentsAdded)
            parts.push(`+${detail.componentsAdded} components`);
        if (detail.componentsRemoved)
            parts.push(`-${detail.componentsRemoved} components`);
        if (detail.connectionsAdded)
            parts.push(`+${detail.connectionsAdded} connections`);
        if (detail.connectionsRemoved)
            parts.push(`-${detail.connectionsRemoved} connections`);
...
function renderMarkdown(record, targetSlug, gatorMemorySlug, createdAt, updatedAt) {
    ...
    const milestoneLines = record.milestones.length > 0
 ...
 */
function buildJsonMirror(record, gatorMemorySlug, createdAt, updatedAt) {
    return {
        ...record,
 ...
 */
export async function mirrorProjectMemory(projectPath) {
    try {
        const config = loadHomeConfig();
        if (!config.memory.mirror.enabled)
            return false;
