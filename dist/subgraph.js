 ...
 */
export function extractSubgraph(components, connections, options = {}) {
    const depth = options.depth ?? 2;
    const maxNodes = options.maxNodes ?? 50;
    let filteredComponentIds = new Set();
    ...
    if (options.focus && options.focus.length > 0) {
        ...
        for (const query of options.focus) {
            ...
            if (resolved)
                focusIds.add(resolved.component_id);
        }
        if (focusIds.size === 0) {
    ...
    // Filter connections
    let filteredConnections = connections.filter(c => filteredComponentIds.has(c.from.component_id) && filteredComponentIds.has(c.to.component_id));
    ...
    const limitedConnections = filteredConnections.filter(c => limitedIds.has(c.from.component_id) && limitedIds.has(c.to.component_id));
    return {
 ...
 */
export function subgraphToMermaid(result) {
