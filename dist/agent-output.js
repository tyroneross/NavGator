...
export function buildExecutiveSummary(components, connections, projectPath, git) {
    const compactComponents = components.map(toCompactComponent);
    const compactConnections = connections.map(toCompactConnection);
...
function computeNextActions(components, risks, blockers) {
    const actions = [];
