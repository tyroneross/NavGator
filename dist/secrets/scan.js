 ...
 */
export function mergeFindings(batches) {
    ...
    for (const { engineId, findings } of batches) {
        for (const finding of findings) {
            ...
            if (!existing.foundBy.includes(engineId))
                existing.foundBy.push(engineId);
    ...
    result.sort((a, b) => {
        if (a.secretClass !== b.secretClass)
            return a.secretClass < b.secretClass ? -1 : 1;
        return a.hash < b.hash ? -1 : a.hash > b.hash ? 1 : 0;
    });
    return result;
}
export async function runSecretScan(paths, opts = {}) {
