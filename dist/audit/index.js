...
import { cochranSize, producersRisk, proportionInterval, sampleWithoutReplacement, selectStratifiedSample, sprtNext, Z, zeroAcceptanceN, zeroDefectUpperBound, } from './sampler.js';
import { newEwmaState, updateChart } from './spc.js';
...
// STRATA KEYS
// ============================================================================
const PACKAGE_TYPES = new Set(['npm', 'pip', 'spm', 'cargo', 'go', 'gem', 'composer']);
const INFRA_TYPES = new Set(['infra', 'database', 'queue', 'cron', 'config']);
export function componentStratum(c) {
...
export function connectionStratum(c) {
...
function precisionDesign(plan, opts, nComp, nConn) {
...
// ============================================================================
function disabledOutcome(cls, ids) {
    return {
        class: cls,
...
async function inspect(comps, conns, files, allConnections, ctx, disabled) {
    ...
    const compOk = new Map();
    for (const e of v1.samples)
        compOk.set(e.id, e);
    const connOk = new Map();
    for (const e of v2.samples)
        connOk.set(e.id, e);
    for (const e of v3.samples) {
        const prev = connOk.get(e.id);
        if (!prev || prev.ok)
            connOk.set(e.id, e); // a connection failing either verifier is one defect
    }
...
export async function runAudit(scanResult, config, projectRoot, opts = {}) {
    ...
    const stratumTotals = {};
    for (const [k, v] of Object.entries(compSample.byStratum))
        stratumTotals[k] = (stratumTotals[k] ?? 0) + v.total;
    for (const [k, v] of Object.entries(connSample.byStratum))
        stratumTotals[k] = (stratumTotals[k] ?? 0) + v.total;
    const pooledStrata = new Set();
    for (const [k, total] of Object.entries(stratumTotals))
        if (total < floor)
            pooledStrata.add(k);
    ...
    const forcedComp = new Set();
    const forcedConn = new Set();
    for (const c of [...screenComps, ...compSample.samples])
        compById.set(c.component_id, c);
    for (const c of [...screenConns, ...connSample.samples])
    ...
    const ctx = { projectRoot, hashes, componentById, isMcpMode: !!opts.isMcpMode };
    ...
    const fileSet = new Set(allFiles.length > 0 ? sampleWithoutReplacement(allFiles, fileSampleCount, rand) : []);
    for (const f of opts.forceInclude?.files ?? [])
        fileSet.add(f);
    ...
    const outcomes = [...insp.outcomes, v5];
    ...
    const obs = [];
    const isDefect = (e) => e !== undefined && !e.ok;
    const isUnverifiable = (e) => e !== undefined && !!e.unverifiable;
    const pushObs = (ids, m) => {
        for (const id of ids) {
            const e = m.get(id);
            if (isUnverifiable(e))
                continue; // F5: no evidence → not an observation
            obs.push(isDefect(e) ? 1 : 0);
        }
    };
    const randomComps = new Set([...compById.keys()].filter((id) => !forcedComp.has(id)));
    const randomConns = new Set([...connById.keys()].filter((id) => !forcedConn.has(id)));
    pushObs(randomComps, insp.compOk);
    pushObs(randomConns, insp.connOk);
    let sprtInfo;
    if (plan === 'SPRT') {
        const cap = Math.min(components.length, FOUNDING_COMPONENTS) +
        ...
        while (step.verdict === 'continue' && obs.length < cap) {
            ...
            if (moreComps.length + moreConns.length === 0)
                break;
            ...
            for (const [id, e] of more.compOk)
                insp.compOk.set(id, e);
            for (const [id, e] of more.connOk)
                insp.connOk.set(id, e);
    ...
    for (const id of randomComps) {
        ...
        if (isUnverifiable(e))
            continue; // F5: excluded from stratum rates
    ...
    for (const v of outcomes) {
        for (const e of v.samples) {
            if (!e.ok && defect_evidence.length < evidenceCap)
                defect_evidence.push({ ...e, class: v.class });
        }
        if (defect_evidence.length >= evidenceCap)
            break;
    }
    // ---- Oracles ----
    let oracles;
    if (opts.oracles !== false) {
        oracles = await runOracles({ projectRoot, components, connections, componentById, hashes }, {
            scip: !!opts.scip,
...
export function updateEwmaForAudit(prior, report, version = NAVGATOR_VERSION) {
    const next = { ...(prior ?? {}) };
    let anyBreach = false;
    ...
    for (const [stratum, stats] of Object.entries(report.by_stratum)) {
        ...
        if (!(stats.sampled > 0))
            continue; // (a): never chart an empty subgroup
    ...
    if (pooled.sampled > 0) {
        ...
        const r = updateChart(prev, { x: pooled.defects / pooled.sampled, n: pooled.sampled, defects: pooled.defects }, { version, kind: 'p' });
