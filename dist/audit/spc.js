 ...
 */
export function updateChart(prev, obs, opts = {}) {
    ...
    const kind = opts.kind ?? base.kind ?? 'p';
    ...
    if (!(sigma > 0))
        sigma = Math.sqrt((floor * (1 - floor)) / LEGACY_N);
    const signals = [];
    // Rule 1 — Shewhart 3σ.
    const ucl = center + 3 * sigma;
    const lcl = Math.max(0, center - 3 * sigma);
    if (x > ucl)
        signals.push('rule1-high');
    ...
    if (lcl > 0 && x < lcl && !floorActive)
        signals.push('rule1-low');
    ...
    if (run_side >= 8)
        signals.push('rule4-high');
    if (run_side <= -8 && !floorActive)
        signals.push('rule4-low');
    ...
    if (ewma > ewma_ucl)
        signals.push('ewma-high');
    if (ewma_lcl > 0 && ewma < ewma_lcl && !floorActive)
        signals.push('ewma-low');
