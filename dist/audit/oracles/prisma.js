...
import { isRootPackageDerived, noOracle, setDiffOracle } from './common.js';
...
export function regexModels(datamodel) {
    const models = [];
...
async function dmmfModels(root, datamodel) {
    let resolved;
    try {
    ...
    if (!getDMMF)
        return null;
    const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error('getDMMF timeout')), DMMF_TIMEOUT_MS).unref?.());
...
export async function loadPrismaTruth(root, opts = {}) {
    ...
    try {
        ...
        if (viaDmmf)
            return { models: viaDmmf, strength: 'independent', files };
    }
    catch (err) {
        return {
            models: regexModels(datamodel),
            strength: 'weak',
            files,
            note: `@prisma/internals getDMMF failed (${err.message}); regex fallback`,
        };
    }
...
export async function prismaOracle(input, opts = {}) {
    const truthInfo = await loadPrismaTruth(input.projectRoot, opts);
    if (!truthInfo)
        return noOracle('prisma', 'infra', 'no prisma schema found (prisma/schema.prisma, schema.prisma, prisma/schema/*.prisma)');
    const truth = new Set(truthInfo.models.map((m) => m.name));
    const tableToModel = new Map();
    for (const m of truthInfo.models)
        if (m.dbName)
            tableToModel.set(m.dbName, m.name);
    const map = new Set();
    const clientLibs = [];
    const notModels = [];
    for (const c of input.components) {
        ...
        if (DATABASE_CLIENT_LIBRARIES.has(c.name.toLowerCase()) || isRootPackageDerived(c))
            clientLibs.push(c.name);
        else
            notModels.push(c.name);
    }
    const notes = [
    ...
    if (truthInfo.note)
        notes.push(truthInfo.note);
    if (clientLibs.length > 0)
        notes.push(`client-library-misclassified: ${clientLibs.join(', ')}`);
    if (notModels.length > 0)
        notes.push(`not-a-schema-model: ${notModels.join(', ')}`);
