...
import { isRootPackageDerived, noOracle, readJsonSafe, setDiffOracle } from './common.js';
export function npmOracle(input) {
    const pkg = readJsonSafe(path.join(input.projectRoot, 'package.json'));
    if (!pkg)
        return noOracle('npm', 'package', 'no readable package.json at project root');
    const truth = new Set([
    ...
    if (truth.size === 0)
        notes.push('package.json declares no dependencies: the frame is empty, so every package-derived component is a false positive');
    if (nested > 0)
        notes.push(`${nested} components from nested package.json files excluded (outside the truth frame)`);
