...
import { noOracle, readJsonSafe, setDiffOracle } from './common.js';
export function cronOracle(input) {
    const vercel = readJsonSafe(path.join(input.projectRoot, 'vercel.json'));
    if (!vercel)
        return noOracle('cron', 'infra', 'no readable vercel.json at project root');
    ...
    for (const c of input.components) {
        if (c.type !== 'cron')
            continue;
        const fromVercel = (c.tags ?? []).includes('vercel') ||
