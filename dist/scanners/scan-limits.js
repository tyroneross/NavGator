...
const DEFAULT_RUN_RE = new RegExp(`[^\\S\\n]{${MAX_WHITESPACE_RUN + 1},}|\\n{${MAX_WHITESPACE_RUN + 1},}`, 'g');
export function collapseWhitespaceRuns(content, maxRun = MAX_WHITESPACE_RUN) {
    const pattern = maxRun === MAX_WHITESPACE_RUN
        ? DEFAULT_RUN_RE
        : new RegExp(`[^\\S\\n]{${maxRun + 1},}|\\n{${maxRun + 1},}`, 'g');
    pattern.lastIndex = 0;
