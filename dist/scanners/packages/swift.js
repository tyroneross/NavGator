...
const SWIFT_FRAMEWORK_SIGNATURES = [
    // UI Frameworks
 ...
 */
export function detectSpm(projectRoot) {
 ...
 */
function parsePackageSwift(content, filePath, timestamp) {
    const components = [];
    const warnings = [];
    ...
    for (const pattern of packagePatterns) {
        ...
        while ((match = pattern.exec(content)) !== null) {
            ...
            components.push(createComponentFromSwiftPackage(name, version, filePath, timestamp));
        }
    }
    ...
    while ((sysMatch = sysLibPattern.exec(content)) !== null) {
        components.push(createComponentFromSwiftPackage(sysMatch[1], undefined, filePath, timestamp));
    }
 ...
 */
function parsePodfile(content, filePath, timestamp) {
    const components = [];
    const warnings = [];
 ...
 */
async function detectFrameworkImports(projectRoot, timestamp, alreadyFound) {
    const components = [];
    ...
    for (const importName of allImports) {
        if (alreadyFound.has(importName))
            continue;
        const signature = SWIFT_FRAMEWORK_SIGNATURES.find((s) => s.packageName === importName);
        if (signature) {
            components.push({
 ...
 */
export async function scanSpmPackages(projectRoot) {
    const components = [];
    const warnings = [];
    ...
    const frameworkComponents = await detectFrameworkImports(projectRoot, timestamp, alreadyFound);
 ...
 */
export function findXcodeProject(projectRoot) {
    try {
        const entries = fs.readdirSync(projectRoot);
 ...
 */
function createComponentFromSwiftPackage(name, version, configFile, timestamp) {
    const signature = SWIFT_FRAMEWORK_SIGNATURES.find((s) => s.packageName.toLowerCase() === name.toLowerCase());
    const type = signature?.type || 'spm';
