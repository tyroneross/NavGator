...
function readMultilineValue(lines, startIndex, firstValue) {
...
function dependencyKindForSection(section) {
...
function parseDependencyLine(line, kind, section, configFile) {
...
function applyDependencyField(dependency, key, raw) {
    if (key === 'package')
        dependency.name = unquote(raw);
    if (key === 'version') {
    ...
    if (key === 'optional')
        dependency.optional = raw === 'true';
    if (key === 'features')
        dependency.features = parseStringArray(raw);
}
function mergeWorkspaceDependency(dependency, workspaceDependencies) {
...
function parseCargoManifest(content, configFile, inheritedWorkspaceDependencies = new Map()) {
    const dependencies = [];
...
async function parseCargoLock(lockPath) {
...
async function resolveCargoManifestPaths(projectRoot, warnings) {
 ...
 */
export async function scanCargoPackages(projectRoot) {
    const components = [];
    const warnings = [];
    ...
    let workspaceDependencies = new Map();
    const rootManifest = path.join(projectRoot, 'Cargo.toml');
    if (fs.existsSync(rootManifest)) {
        try {
    ...
    for (const manifestPath of manifestPaths) {
        try {
            ...
            for (const dependency of parsed.dependencies) {
                ...
                components.push(createComponentFromCargoDependency(dependency, lockInfo, timestamp));
            }
        }
        catch (error) {
            warnings.push({
 ...
 */
export function detectCargo(projectRoot) {
...
// HELPERS
// =============================================================================
function createComponentFromCargoDependency(dependency, lockInfo, timestamp) {
    const signature = RUST_SIGNATURES.find((s) => s.packageName.toLowerCase() === dependency.name.toLowerCase());
    const type = signature?.type || 'cargo';
