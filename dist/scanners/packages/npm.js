 ...
 */
async function resolveWorkspacePackages(projectRoot) {
    const packagePaths = [];
 ...
 */
async function scanSinglePackageJson(packageJsonPath, warnings) {
    const components = [];
    let packageJson;
    try {
        const content = await fs.promises.readFile(packageJsonPath, 'utf-8');
    ...
    if (packageJson.dependencies) {
        for (const [name, version] of Object.entries(packageJson.dependencies)) {
            components.push(createComponentFromPackage(name, version, 'core', packageJsonPath, timestamp));
        }
    }
    if (packageJson.devDependencies) {
        for (const [name, version] of Object.entries(packageJson.devDependencies)) {
            components.push(createComponentFromPackage(name, version, 'dev', packageJsonPath, timestamp));
        }
    }
    return components;
}
 ...
 */
export async function scanNpmPackages(projectRoot) {
    const components = [];
    const warnings = [];
    const packageJsonPath = path.join(projectRoot, 'package.json');
    if (!fs.existsSync(packageJsonPath)) {
 ...
 */
export function detectNpm(projectRoot) {
