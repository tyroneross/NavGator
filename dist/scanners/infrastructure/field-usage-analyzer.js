 ...
 */
function scanFieldInFiles(fieldName, columnName, files // filename -> content
) {
    const readFiles = [];
    const writeFiles = [];
    const allFilesSet = new Set();
    // Build search terms: camelCase name + snake_case @map name (if different)
    const searchTerms = new Set([fieldName]);
    ...
    for (const [filePath, content] of files) {
        ...
        for (const term of searchTerms) {
            ...
            const fieldRegex = new RegExp(`(?:^|[^a-zA-Z0-9_])${escapeRegex(term)}(?:[^a-zA-Z0-9_]|$)`, 'gm');
 ...
 */
function classifyUsage(allFiles, readFiles, writeFiles) {
    ...
    if (readFiles.length > 0 && writeFiles.length === 0)
        return 'read-only';
    if (writeFiles.length > 0 && readFiles.length === 0)
        return 'write-only';
 ...
 */
export async function scanFieldUsage(projectRoot, walkSet) {
    const components = [];
    const connections = [];
    const warnings = [];
    ...
    for (const model of models) {
        ...
        for (const field of model.fields) {
            ...
            const { readFiles, writeFiles, allFiles } = scanFieldInFiles(field.name, field.columnName, fileContents);
        ...
        const allReferencingFiles = new Set();
        for (const fr of fieldRecords) {
            for (const f of fr.allFiles)
                allReferencingFiles.add(f);
        }
    ...
    return { components, connections, warnings, report };
}
/**
 * Detect if field usage analysis is possible (requires Prisma schema)
 */
export function canAnalyzeFieldUsage(projectRoot) {
 ...
 */
export function formatFieldUsageReport(report) {
    const lines = [];
...
//# sourceMappingURL=field-usage-analyzer.js.map