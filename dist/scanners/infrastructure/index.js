 ...
 */
export async function scanInfrastructure(projectRoot) {
    const components = [];
    const warnings = [];
    ...
    for (const signature of INFRA_SIGNATURES) {
        ...
        if (detected) {
            const component = createInfraComponent(signature, detected.configFiles, timestamp);
 ...
 */
async function detectInfra(projectRoot, signature) {
    const foundFiles = [];
 ...
 */
function createInfraComponent(signature, configFiles, timestamp) {
    return {
 ...
 */
export async function parseDockerCompose(projectRoot) {
    const services = [];
 ...
 */
export async function parseRailwayConfig(projectRoot) {
 ...
 */
export function hasInfrastructure(projectRoot) {
    for (const signature of INFRA_SIGNATURES) {
        for (const file of signature.files) {
