 ...
 */
export function parseConnectionUrl(value) {
 ...
 */
function inferResourceType(protocol) {
    switch (protocol) {
        case 'postgres':
        case 'postgresql':
        case 'mysql':
 ...
 */
function parseEnvFileWithValues(content) {
 ...
 */
async function findEnvReferences(projectRoot, walkSet) {
    const envRefs = new Map(); // envVar -> [file1, file2, ...]
...
export function categorizeEnvVar(name) {
 ...
 */
export async function scanEnvVars(projectRoot, walkSet) {
    const components = [];
    const connections = [];
    const warnings = [];
    ...
    const definedVars = new Map(); // varName -> [envFile1, envFile2]
    // Collect raw values only for URL parsing — never surfaced to callers
    const envValues = new Map(); // varName -> raw value
    for (const envFile of envFiles) {
        try {
            const content = await fs.promises.readFile(path.join(projectRoot, envFile), 'utf-8');
    ...
    for (const varName of allVarNames) {
        ...
        if (definedIn.length === 0)
            continue; // source-only → skip; warning emitted in Phase 4
    ...
    const envComponentMap = new Map(); // varName -> component_id
    ...
    for (const [varName] of envRefs) {
        if (!definedVars.has(varName)) {
            ...
            if (skipPatterns.some(p => varName.startsWith(p)))
                continue;
            warnings.push({
                type: 'missing_file',
 ...
 */
export function detectEnvFiles(projectRoot) {
