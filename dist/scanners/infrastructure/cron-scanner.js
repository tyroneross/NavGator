 ...
 */
function parseVercelCrons(projectRoot) {
    const crons = [];
    const warnings = [];
 ...
 */
function parseRailwayCrons(projectRoot) {
    const crons = [];
    const warnings = [];
    // railway.json
 ...
 */
async function findCodeCrons(projectRoot, walkSet) {
    const crons = [];
    const warnings = [];
 ...
 */
function resolveApiRoute(projectRoot, cronPath) {
 ...
 */
export async function scanCronJobs(projectRoot, walkSet) {
    const components = [];
    const connections = [];
    const warnings = [];
    ...
    warnings.push(...vercelResult.warnings, ...railwayResult.warnings, ...codeResult.warnings);
 ...
 */
export function detectCrons(projectRoot) {
    ...
    if (fs.existsSync(railwayToml)) {
        try {
            ...
            if (content.includes('cronSchedule'))
                return true;
        }
        catch {
            // Ignore
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
            const allDeps = { ...pkg.dependencies, ...pkg.devDependencies };
            if (allDeps['node-cron'] || allDeps.cron)
                return true;
        }
        catch {
            // Ignore
...
//# sourceMappingURL=cron-scanner.js.map