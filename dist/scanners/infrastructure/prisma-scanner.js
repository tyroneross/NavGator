 ...
 */
function findPrismaSchemas(projectRoot) {
    const candidates = [
 ...
 *   }
 */
export function parseDatasource(content) {
 ...
 */
export async function scanPrismaSchema(projectRoot) {
    const components = [];
    const connections = [];
    const warnings = [];
    ...
    const datasourceByFile = new Map();
    for (const schemaFile of schemaFiles) {
        try {
            const content = await fs.promises.readFile(path.join(projectRoot, schemaFile), 'utf-8');
            const models = parsePrismaSchema(content);
            for (const model of models) {
                model._sourceFile = schemaFile;
            }
    ...
    const modelComponentMap = new Map(); // modelName -> component_id
    for (const model of allModels) {
        const sourceFile = model._sourceFile || schemaFiles[0];
 ...
 */
export function detectPrisma(projectRoot) {
