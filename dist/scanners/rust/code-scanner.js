...
export async function scanRustCode(projectRoot, walkSet, knownPackages) {
    ...
    for (const t of typeDecls) {
        ...
        if (!declFilesByCrateName.has(key))
            declFilesByCrateName.set(key, new Set());
        declFilesByCrateName.get(key).add(t.file);
        if (!nodeFileByName.has(t.name))
            nodeFileByName.set(t.name, t.file);
    }
    const nodeIdForType = (name) => idFor('component', name) ?? idFor('other', name);
    for (const file of files) {
        const crate = crateForFile(file.relativePath, crates);
        ...
        for (const ref of scanRustTypeUses(file)) {
            ...
            if (!nodeId && declFile === file.relativePath)
                continue; // no node to attribute a same-file use to
 ...
 * do not use it.
 */
function scanRustTypeUses(file) {
