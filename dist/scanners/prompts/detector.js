 ...
 */
export function findArrayEnd(lines, startLine) {
    let depth = 0;
    let inString = false;
    let stringChar = '';
...
export class PromptDetector {
     ...
     */
    async scanProject(projectRoot, walkSet) {
        const prompts = [];
        const warnings = [];
        // Find source files
    ...
    detectInFile(filePath, content) {
        ...
        for (const match of promptMatches) {
            ...
            const signals = this.countCorroborationSignals(match, lines, apiCalls, hasAIImport, hasTraceableDecorator);
     ...
     */
    countCorroborationSignals(match, lines, apiCalls, hasAIImport, hasTraceableDecorator) {
        let signals = 0;
        ...
        const nearbyCall = apiCalls.find(c => Math.abs(c.line - match.lineStart) < 50);
        if (nearbyCall)
            signals++;
        // Signal 4: @traceable() decorator in file
        if (hasTraceableDecorator)
            signals++;
        ...
        if (match.name) {
            ...
            if (strongNames.test(match.name))
                signals++;
        }
     ...
     */
    findPromptDefinitions(content, lines) {
        const matches = [];
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            ...
            const varMatch = line.match(/(?:const|let|var)\s+(\w*[Pp]rompt\w*|\w*[Ii]nstruction\w*|SYSTEM_PROMPT|USER_PROMPT)\s*=\s*([`'"])/);
            if (varMatch) {
            ...
            const propMatch = line.match(/(?:systemPrompt|system_prompt)\s*:\s*([`'"])/i);
            if (propMatch) {
     ...
     */
    findPythonStringEnd(lines, startLine) {
     ...
     */
    buildPromptFromMatch(filePath, lines, match, apiCalls) {
     ...
     */
    findNearestAPICall(apiCalls, promptLine) {
        let nearest;
     ...
     */
    extractPurpose(lines, lineIndex, content) {
    ...
    detectTemplateSyntax(content) {
        ...
        if (/\{\{\{/.test(content))
            return 'mustache';
        if (/f['"]/.test(content) || /f"""/.test(content))
            return 'fstring';
     ...
     */
    generateTags(content, messages) {
        const tags = [];
        const lowerContent = content.toLowerCase();
        if (messages.some(m => m.role === 'system'))
            tags.push('has-system-prompt');
        ...
        if (lowerContent.includes('step by step') || lowerContent.includes('step-by-step'))
            tags.push('chain-of-thought');
        if (lowerContent.includes('tool') || lowerContent.includes('function'))
            tags.push('tool-use');
     ...
     */
    detectCategory(content, messages) {
        const lowerContent = content.toLowerCase();
        if (lowerContent.includes('summarize') || lowerContent.includes('summary'))
            return 'summarization';
        if (lowerContent.includes('extract') || lowerContent.includes('parse'))
            return 'extraction';
        if (lowerContent.includes('classify') || lowerContent.includes('categorize'))
            return 'classification';
        if (lowerContent.includes('translate'))
            return 'translation';
        if (lowerContent.includes('code') && (lowerContent.includes('write') || lowerContent.includes('generate')))
            return 'code-generation';
        if (lowerContent.includes('review') && lowerContent.includes('code'))
            return 'code-review';
        if (lowerContent.includes('tool') || lowerContent.includes('function_call'))
            return 'agent';
        if (lowerContent.includes('embed'))
            return 'embedding';
        if (messages.length > 1 || lowerContent.includes('chat') || lowerContent.includes('conversation'))
            return 'chat';
        return 'unknown';
    }
    /**
     ...
     */
    calculateConfidence(messages, apiCall) {
     ...
     */
    generatePromptId(file, line, name) {
     ...
     */
    generatePromptName(file, functionName, line) {
