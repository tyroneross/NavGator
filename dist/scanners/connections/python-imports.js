 ...
 */
function cleanPythonLines(content) {
    ...
    while (i < lines.length) {
        ...
        if (plainMatch) {
            for (const rawEntry of plainMatch[1].split(',')) {
                ...
                if (!modulePath || modulePath === '__future__')
                    continue;
                specs.push({
 ...
 */
function candidateSegmentSets(spec) {
    ...
    if (moduleSegs.length > 0)
        candidates.push(moduleSegs);
    return candidates;
}
function tryCandidates(root, candidates, knownFiles) {
...
const rootCandidateCache = new WeakMap();
function ancestorRoots(knownFiles) {
    ...
    const ordered = [...roots].sort((a, b) => a.split('/').length - b.split('/').length || a.localeCompare(b));
 ...
 */
export function resolvePythonImport(spec, importerFile, knownFiles) {
 ...
 */
export function pythonPackageHead(spec) {
    if (spec.relativeLevel > 0)
        return null;
    const head = spec.modulePath.split('.')[0];
    if (!head)
        return null;
    if (PYTHON_STDLIB_MODULES.has(head))
        return null;
    return head;
}
//# sourceMappingURL=python-imports.js.map