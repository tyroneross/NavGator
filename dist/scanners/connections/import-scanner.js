...
function resolveAliasTargets(specifier, pathAliases) {
...
function isConfiguredAlias(specifier, pathAliases) {
...
function resolveImport(specifier, importerDir, projectRoot, knownFiles, pathAliases = []) {
    ...
    for (const absTarget of absoluteTargets) {
        ...
        if (isOutsideProject(relativeTarget))
            continue;
 ...
 */
function loadPathAliasesFromConfig(configPath) {
        ...
        if (paths && typeof paths === 'object') {
            for (const [pattern, targets] of Object.entries(paths)) {
                ...
                if (absoluteTargets.length > 0)
                    aliases.push({ pattern, absoluteTargets });
            }
        }
        if (aliases.length > 0) {
...
function loadPathAliasesForImporter(importerFile, projectRoot, cache) {
    ...
    while (true) {
        ...
        if (parentDir === currentDir || isOutsideProject(path.relative(root, parentDir)))
            break;
 ...
 * already skip nulls.
 */
async function readBoundedFile(projectRoot, file) {
    try {
...
export async function scanImports(projectRoot, sourceFiles, knownPackages) {
        ...
        for (const result of results) {
            ...
            if (packageIdByName.size > 0) {
                ...
                for (const rawSpec of bareSpecs) {
                    ...
                    // leading segment.
                    if (isConfiguredAlias(rawSpec, pathAliases))
                        continue;
    ...
    for (let i = 0; i < files.length; i += batchSize) {
        ...
        for (const result of results) {
            ...
            const isFrontend = file.startsWith('app/') || file.includes('/app/') ||
