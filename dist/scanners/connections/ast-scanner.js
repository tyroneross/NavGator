 ...
 */
export async function scanWithAST(projectRoot, walkSet) {
    const components = [];
    const connections = [];
    const warnings = [];
    ...
    for (const sourceFile of project.getSourceFiles()) {
        ...
        for (const signature of SERVICE_SIGNATURES) {
            const serviceImport = imports.find(imp => signature.importPatterns.some(pattern => imp.moduleName === pattern || imp.moduleName.startsWith(pattern + '/')));
            if (serviceImport) {
                ...
                sourceFile.forEachDescendant((node) => {
                    if (Node.isCallExpression(node)) {
                        ...
                        const involvesImport = identifiersToTrack.some(id => expressionText.startsWith(id) || expressionText.includes(`.${id}`));
                        if (involvesImport) {
 ...
 */
export async function scanDatabaseOperations(projectRoot, walkSet) {
    const components = [];
    const connections = [];
    const warnings = [];
    const timestamp = Date.now();
