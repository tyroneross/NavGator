 ...
 */
export function commitScan(storeDir, message) {
...
export function listSnapshots(storeDir, limit = 50) {
    if (!isInitialized(storeDir))
        return [];
 ...
 */
export function findFirstSeen(storeDir, needle) {
    if (!isInitialized(storeDir))
        return null;
 ...
 */
export function diffSince(storeDir, ref) {
    if (!isInitialized(storeDir))
        return { ok: false, stat: '', error: 'store not initialized' };
    const r = runGit(storeDir, ['diff', '--stat', `${ref}..HEAD`]);
    if (!r.ok)
        return { ok: false, stat: '', error: r.stderr.trim() };
 ...
 */
export function showFileAt(storeDir, sha, relPath) {
    if (!isInitialized(storeDir))
