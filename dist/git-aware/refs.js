 ...
 */
export async function getDefaultBranch(root) {
    ...
    if (await localBranchExists(root, 'main'))
        return 'main';
    if (await localBranchExists(root, 'master'))
        return 'master';
    return null;
}
 ...
 */
export async function getCurrentRef(root) {
...
export async function isDefaultBranch(root) {
...
export function slugifyRef(ref) {
    ...
    if (truncated === ref && truncated !== '.' && truncated !== '..')
        return truncated;
