...
export async function discoverSourceFiles(root) {
    ...
    try {
        ...
        if (names.length > 0)
            tracked = new Set(names);
    }
    catch {
        tracked = null;
    }
    if (tracked === null) {
        process.stderr.write('arch-index: git could not list tracked files; falling back to filesystem glob. ' +
 ...
 */
export function autoModuleId(file) {
...
function assignModule(file, curated) {
    let best = null;
    for (const mod of curated) {
        ...
        if (!matches)
            continue;
        if (!best || mod.path.length > best.path.length)
            best = mod;
    }
    if (best)
        return { id: best.id, curated: true };
    return { id: autoModuleId(file), curated: false };
}
export function loadCuratedModules(root) {
...
export async function buildArchitectureIndex(root) {
    ...
    blindSpots.push('Only static import/require/re-export edges are indexed. Runtime wiring — dependency ' +
        'injection, string-keyed registries, HTTP calls, queue topics — is not.');
    blindSpots.push('Edges come from matching import syntax in raw file text, not from a compiler, so a ' +
    ...
    blindSpots.sort();
    const status = internalEdges === 0 ? 'none' : unmeasured ? 'partial' : 'full';
    const coverage = {
        status,
    ...
    const filesInModule = new Map();
    for (const id of moduleIds)
        filesInModule.set(id, []);
    for (const file of analyzedFiles)
        filesInModule.get(moduleOfFile.get(file)).push(file);
    // --- module edges -------------------------------------------------------
    const moduleEdgeCount = new Map();
    ...
    for (const file of analyzedFiles) {
        ...
        for (const target of [...importsOf.get(file)].sort()) {
            ...
            if (!boundaryEvidence.has(key))
                boundaryEvidence.set(key, []);
    ...
    // --- boundaries ---------------------------------------------------------
    const boundaries = curatedBoundaries.map(rule => {
...
export function renderArchitectureMarkdown(index) {
    const lines = [];
    ...
    push();
    for (const spot of index.coverage.blind_spots)
        push(`- ${spot}`);
    push();
    // --- modules ------------------------------------------------------------
    push('## Modules');
...
export async function writeArchitectureIndex(root) {
...
//# sourceMappingURL=architecture-index.js.map