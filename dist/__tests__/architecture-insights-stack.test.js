...
function buildLinearChain(depth) {
    ...
    for (let i = 0; i < depth - 1; i++) {
        connections.push(createConnection(components[i], components[i + 1], { connection_type: 'imports' }));
    }
...
describe('detectImportCycles — stack-overflow regression', () => {
    ...
    it('detects a single cycle inside a 15,000-node graph', () => {
        ...
        connections.push(createConnection(components[components.length - 1], components[0], {
    ...
    it('respects the limit when many cycles are present', () => {
        ...
        const connections = [];
        for (let r = 0; r < 10; r++) {
            const ring = Array.from({ length: 4 }, (_, i) => createComponent({
                name: `ring-${r}/node-${i}`,
                type: 'component',
            ...
            for (let i = 0; i < ring.length; i++) {
                connections.push(createConnection(ring[i], ring[(i + 1) % ring.length], { connection_type: 'imports' }));
            }
        }
...
//# sourceMappingURL=architecture-insights-stack.test.js.map