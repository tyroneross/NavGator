...
import { detectImportCycles, detectLayerViolations, detectShallowModules, getModuleDepthSignals, getTopFanOut, getTopHotspots, } from '../architecture-insights.js';
...
describe('architecture insights', () => {
    ...
    it('does NOT flag a deep module (high fan-in, low fan-out) as shallow', () => {
        ...
        const core = createComponent({ name: 'core/types', type: 'component', file: 'src/core/types.ts' });
        const importers = Array.from({ length: 5 }, (_, index) => createComponent({ name: `cdp/dep-${index}`, type: 'component', file: `src/cdp/dep-${index}.ts` }));
        const shallow = detectShallowModules([core, ...importers], importers.map((dep) => createConnection(dep, core, { connection_type: 'imports' })));
    ...
    it('flags a shallow module (imports many, used by few)', () => {
        ...
        const deps = Array.from({ length: 6 }, (_, index) => createComponent({ name: `core/dep-${index}`, type: 'component', file: `src/core/dep-${index}.ts` }));
        const shallow = detectShallowModules([glue, app, ...deps], [
    ...
    it('computes fanIn / fanOut / shallownessScore on a known graph', () => {
        ...
        const signals = getModuleDepthSignals([a, b, c, d], [
            createConnection(a, b, { connection_type: 'imports' }),
