...
describe('coverage', () => {
    ...
    const mockConnections = [
        ...
        createMockConnection('comp-1', 'comp-3', { connection_id: 'conn-2', confidence: 0.4, semantic: { classification: 'api-call', confidence: 0.8 } }),
        createMockConnection('comp-2', 'comp-1', { connection_id: 'conn-3', confidence: 0.6, semantic: { classification: 'data-query', confidence: 0.8 } }),
    ];
    const mockFileMap = {
    ...
    it('handles empty gaps gracefully', async () => {
        ...
        const report = await computeCoverage(noGapComponents, noGapConnections, projectRoot, { 'src/a.ts': 'comp-1', 'src/b.ts': 'comp-2' });
        // Real project files will show as unmapped, but component-level gaps should be empty
        const componentGaps = report.gaps.filter(g => g.type === 'zero-consumers' || g.type === 'no-outgoing' || g.type === 'low-confidence-connection');
