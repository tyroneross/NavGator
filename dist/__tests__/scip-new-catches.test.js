...
async function evaluateCase(caseName) {
    ...
    const regexPairs = new Set(regexImports.map((c) => {
        const from = c.from?.location?.file ?? '';
        const to = c.code_reference?.file ?? '';
...
describe('SCIP new-catches (Wave 2 T12)', () => {
    it.skipIf(!fs.existsSync(FIXTURE_ROOT))('catches ≥3/5 patterns the regex import-scanner misses', { timeout: 30_000 }, async () => {
        const results = [];
...
//# sourceMappingURL=scip-new-catches.test.js.map