...
function makeTmpProject() {
    ...
    fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ name: 'incremental-fixture', version: '0.0.0', dependencies: {} }, null, 2));
    fs.writeFileSync(path.join(root, 'src', 'a.ts'), `import { fromB } from './b';\nexport function fromA() { return fromB() + 1; }\n`);
    fs.writeFileSync(path.join(root, 'src', 'b.ts'), `import { fromC } from './c';\nexport function fromB() { return fromC() + 1; }\n`);
    fs.writeFileSync(path.join(root, 'src', 'c.ts'), `export function fromC() { return 1; }\n`);
    return root;
}
function emptyChanges() {
    return { added: [], modified: [], removed: [], unchanged: [] };
}
function indexWith(overrides = {}) {
    return {
...
describe('selectScanMode (Run 1 — D2)', () => {
    ...
    it('build-config-trigger: vercel.json modified → full / manifest-changed', () => {
        const decision = selectScanMode({ ...emptyChanges(), modified: ['vercel.json'] }, indexWith(), {});
        expect(decision.mode).toBe('full');
        expect(decision.reason).toBe('manifest-changed');
    });
    it('build-config-trigger: .gitignore modified → full / manifest-changed', () => {
        const decision = selectScanMode({ ...emptyChanges(), modified: ['.gitignore'] }, indexWith(), {});
        expect(decision.mode).toBe('full');
        expect(decision.reason).toBe('manifest-changed');
    });
    // Run 1.6 — item #5: new files force full scan (no recorded reverse-dep edges)
    it('new-files: any added file → full / new-files', () => {
        const decision = selectScanMode({ ...emptyChanges(), added: ['src/new-leaf.ts'] }, indexWith(), {});
        expect(decision.mode).toBe('full');
        expect(decision.reason).toBe('new-files');
    });
    it('new-files: added + modified mix still → full / new-files', () => {
        const decision = selectScanMode({ ...emptyChanges(), added: ['src/new-leaf.ts'], modified: ['src/a.ts'] }, indexWith(), {});
        expect(decision.mode).toBe('full');
        expect(decision.reason).toBe('new-files');
    });
    it('stale-trigger: last_full_scan 8 days ago → full / stale-full', () => {
        const eightDaysAgo = Date.now() - 8 * 24 * 60 * 60 * 1000;
        const decision = selectScanMode({ ...emptyChanges(), modified: ['src/a.ts'] }, indexWith({ last_full_scan: eightDaysAgo }), {});
        expect(decision.mode).toBe('full');
        expect(decision.reason).toBe('stale-full');
    });
    it('incremental-cap: incrementals_since_full=20 → full / incremental-cap', () => {
        const decision = selectScanMode({ ...emptyChanges(), modified: ['src/a.ts'] }, indexWith({ incrementals_since_full: 20 }), {});
        expect(decision.mode).toBe('full');
    ...
    it('fast-path: a code file modified, recent full, low incrementals → incremental / fast-path', () => {
        const decision = selectScanMode({ ...emptyChanges(), modified: ['src/a.ts'] }, indexWith(), {});
    ...
    it('lockfile-trigger overrides incremental flag (treated as auto by spec? No — flag wins for explicit incremental)', () => {
        ...
        const changes = { ...emptyChanges(), modified: ['package.json'] };
...
describe('incremental scan e2e (Run 1 — D4)', () => {
    ...
    it('scenario 5: integrity-auto-promote → corrupt connection target → incremental→full', async () => {
        ...
        const cfgArg = { storageMode: 'local', storagePath: '.navgator/architecture', autoScan: false, healthCheckEnabled: false, scanDepth: 'shallow', defaultConfidenceThreshold: 0.6 };
    ...
    it('scenario 4: incremental-cap → simulate index with incrementals_since_full=20 → full', async () => {
        ...
        const idxPath = path.join(getStoragePath({ storageMode: 'local', storagePath: '.navgator/architecture', autoScan: false, healthCheckEnabled: false, scanDepth: 'shallow', defaultConfidenceThreshold: 0.6 }, projectRoot), 'index.json');
...
describe('scan concurrency lock (Run 1.6 — item #4)', () => {
    ...
    it('held lock → second scan exits cleanly with the contention message', async () => {
        ...
        fs.writeFileSync(lockPath, JSON.stringify({ pid: process.pid, started_at: Date.now(), scan_type: 'incremental' }));
...
describe('schema migration 1.0.0 → 1.1.0 (Run 1 — C14)', () => {
    it('loadIndex injects defaults for 1.0.0 archive', async () => {
        ...
        fs.writeFileSync(path.join(root, '.navgator', 'architecture', 'index.json'), JSON.stringify(oldIndex));
...
describe('walk-set restriction (Run 1.5)', () => {
    it('scanServiceCalls reads only files in walkSet when provided', async () => {
        ...
        const walkSet = new Set(['src/a.ts']);
    ...
    it('scanWithAST loads only walkSet files into the ts-morph project (best-effort)', async () => {
        ...
        const result = await scanWithAST(root, new Set());
...
describe("aliased imports resolve to project paths (Run 1.6 — item #7)", () => {
    it("tsconfig paths alias resolves at scan time; connection target stores resolved path", async () => {
        ...
        fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "alias-fixture", version: "0.0.0", dependencies: {} }));
        ...
        fs.writeFileSync(path.join(root, "src", "utils", "foo.ts"), "export const x = 1;\n");
        fs.writeFileSync(path.join(root, "src", "index.ts"), "import { x } from \"@/utils/foo\";\nexport const y = x + 1;\n");
        ...
        const importers = await loadReverseDeps(new Set(["src/utils/foo.ts"]), undefined, root);
...
describe('integrity-promote no-truncation (Run 1.7 — Problem A)', () => {
    it('promote retains full graph (>= baseline counts), not just walk-set slice', async () => {
        ...
        const corruptPath = path.join(connectionsDir, chosen);
...
describe('dedup-by-name cross-type collision (Run 1.7 — Problem B)', () => {
    it('file component "prisma" coexists with database component "prisma"', async () => {
        ...
        fs.mkdirSync(path.join(root, 'src'), { recursive: true });
        fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ name: 'prob-b-fixture', version: '0.0.0', dependencies: { prisma: '^5.0.0' } }, null, 2));
        fs.writeFileSync(path.join(root, 'lib', 'prisma.ts'), `export const client = { query: () => null };\n`);
        fs.writeFileSync(path.join(root, 'src', 'a.ts'), `import { client } from '../lib/prisma';\nexport const x = client;\n`);
        ...
        const importEdges = result.connections.filter((c) => c.connection_type === 'imports' &&
...
//# sourceMappingURL=scanner-incremental.test.js.map