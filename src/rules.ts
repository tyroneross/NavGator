    ...
    },
    {
      ...
      check: (components, connections) => {
        ...
        for (const conn of connections) {
          ...
          if (conn.connection_type === 'conforms-to') continue;
          dependentCounts.set(
...
function analyzeTransitiveDeadCode(
  ...
  // unchanged.
  const sameFileIds = new Map<string, string[]>();
  ...
  for (const c of components) {
    ...
    for (const p of paths) {
      ...
      else sameFileIds.set(p, [c.component_id]);
    }
  }
  ...
  while (queue.length > 0) {
    ...
    for (const p of pathsOf.get(current) ?? []) neighbors.push(...(sameFileIds.get(p) ?? []));
