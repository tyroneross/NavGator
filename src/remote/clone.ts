 ...
 */
function realpathOfDeepestExistingAncestor(target: string): string {
  let current = target;
...
function assertInsideCacheRoot(dest: string, cacheRoot: string): void {
  ...
  if (realDest !== realRoot && !realDest.startsWith(realRoot + path.sep)) {
    throw new Error(
      `Refusing to clone outside the cache root: a symlink resolves destination "${dest}" to "${realDest}", which escapes "${realRoot}"`
    );
  }
}
