...
const ENTRY_POINT_TYPES: ReadonlySet<string> = new Set([
  'api-endpoint',
  'worker',
  'cron',
  'xcode-target',
...
const ENTRY_POINT_TAGS: ReadonlySet<string> = new Set([
  'entrypoint',
  'route',
  'navigation-root',
]);
 ...
 */
const PATH_CONVENTIONS: ReadonlyArray<readonly [EntryPointSource, RegExp]> = [
...
export function entryCandidatePaths(
  ...
  const metaFile = (component.metadata as Record<string, unknown> | undefined)?.['file'];
  if (typeof metaFile === 'string') raw.push(metaFile);
 ...
 */
export function classifyPathConvention(filePath: string): EntryPointSource | null {
  for (const [source, pattern] of PATH_CONVENTIONS) {
    if (pattern.test(filePath)) return source;
  }
  return null;
}
 ...
 */
export function declaredEntryPaths(manifest: unknown, dir = ''): DeclaredPath[] {
  if (!manifest || typeof manifest !== 'object') return [];
  const pkg = manifest as Record<string, unknown>;
  const out: DeclaredPath[] = [];
  const add = (value: unknown, source: EntryPointSource): void => {
  ...
  if (typeof pkg['bin'] === 'string') add(pkg['bin'], 'package-entry');
  ...
  if (pkg['scripts'] && typeof pkg['scripts'] === 'object') {
    for (const command of Object.values(pkg['scripts'] as Record<string, unknown>)) {
      if (typeof command !== 'string') continue;
      for (const token of command.split(/[\s'"`;=(),]+/)) {
        if (SCRIPT_PATH_TOKEN.test(token)) add(token, 'package-script');
      }
    }
  }
 ...
 */
export function resolveDeclaredTarget(target: string): string[] {
  const bases = new Set<string>([target]);
 ...
 */
function manifestDirs(components: ArchitectureComponent[], projectRoot: string): string[] {
  const dirs = new Set<string>(['']);
  for (const component of components) {
    const origin = (component.metadata as Record<string, unknown> | undefined)?.['origin_root'];
...
export function detectEntryPoints(
  ...
  for (const component of components) {
    for (const filePath of entryCandidatePaths(component, projectRoot)) {
      ...
      if (list) list.push(component.component_id);
      else idsByPath.set(filePath, [component.component_id]);
    }
  }
      ...
      for (const { target, source } of declaredEntryPaths(manifest, dir)) {
        ...
        for (const candidate of resolveDeclaredTarget(target)) {
          for (const id of idsByPath.get(candidate) ?? []) {
            if (!reasons.has(id)) reasons.set(id, source);
          }
        }
      }
    }
  ...
  for (const component of components) {
    ...
    let matched: EntryPointSource | null = null;
    for (const filePath of entryCandidatePaths(component, projectRoot)) {
  ...
  const counts: Partial<Record<EntryPointSource, number>> = {};
