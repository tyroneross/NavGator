...
import { noOracle, setDiffOracle, type OracleInput, type OracleResult } from './common.js';

function norm(p: string | undefined): string | null {
  if (!p) return null;
  let s = p.replace(/\\/g, '/');
...
export async function importsScipOracle(input: OracleInput, opts: { enabled: boolean; timeoutMs?: number }): Promise<OracleResult> {
  if (!opts.enabled) return noOracle('imports-scip', 'connection-imports', 'skipped: run `navgator scan --scip` or `navgator audit-report --scip` to enable');

  const { runScip, crossFileEdges, hasTsConfig } = await import('../../parsers/scip-runner.js');
  if (!hasTsConfig(input.projectRoot)) return noOracle('imports-scip', 'connection-imports', 'no tsconfig.json / jsconfig.json at project root');
  ...
  if (input.hashes?.files) {
    for (const f of Object.keys(input.hashes.files)) if (/\.(ts|tsx|mts|cts)$/.test(f)) tsFiles++;
  }
  ...
  if (coverage !== undefined) res.frame_coverage = coverage;
  return res;
}
