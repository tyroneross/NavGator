...
import { isRootPackageDerived, noOracle, setDiffOracle, walkSourceFiles, type OracleInput, type OracleResult } from './common.js';
 ...
 */
function resolveIdent(ident: string, sameFile: string, corpus: ReadonlyArray<string>): string | null {
  const parts = ident.split('.');
  if (parts.length === 1) {
    const re = new RegExp(`\\b${escapeRe(ident)}\\s*(?::\\s*string)?\\s*=\\s*(['"\`])([^'"\`$\\n]+)\\1`);
    const local = re.exec(sameFile);
    if (local) return local[2]!;
    ...
    if (!/^[A-Z][A-Z0-9_]+$/.test(ident)) return null;
...
export function queueLiterals(root: string): { names: Set<string>; files: number; unresolved: string[] } {
  const names = new Set<string>();
  const unresolved: string[] = [];
  const files = walkSourceFiles(root);
  const contents: string[] = [];
  const hinted: Array<{ content: string }> = [];
...
export function queueOracle(input: OracleInput): OracleResult {
  const { names: truth, files, unresolved } = queueLiterals(input.projectRoot);
  const map = new Set<string>();
  for (const c of input.components) {
    if (c.type !== 'queue') continue;
    if (isRootPackageDerived(c)) continue;
    if ((c.source?.config_files ?? []).some((f) => path.basename(f) === 'package.json')) continue;
    map.add(c.name);
  }
  if (truth.size === 0 && map.size === 0) return noOracle('queue', 'infra', `no queue literals in ${files} source files and no queue components`);
