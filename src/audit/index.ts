...
export interface AuditReport {
  ...
  defect_rate: number;
  by_class: Partial<Record<DefectClass, { sampled: number; defects: number }>>;
...
const PACKAGE_TYPES = new Set<ComponentType>([
  'npm',
  'pip',
  'spm',
  'cargo',
...
const INFRA_TYPES = new Set<ComponentType>([
  'infra',
  'database',
  'queue',
  'cron',
...
function componentStratum(c: ArchitectureComponent): string {
...
function connectionStratum(c: ArchitectureConnection): string {
...
function pickPlan(opts: AuditOptions, lotSize: number): {
  plan: AuditPlan;
  n: number;
  c: number;
} {
...
export async function runAudit(
  ...
  const componentSamples = selectAuditSample(
    components,
    Math.ceil(planN * 0.5),
    componentStratum,
    undefined,
  ...
  const connectionSamples = selectAuditSample(
    connections,
    Math.floor(planN * 0.5),
    connectionStratum,
    undefined,
    opts.rand
  );
  ...
  const verifiers: VerifierOutcome[] = [v1, v2, v3, v4, v5, v6];
  ...
  const stratumAgg: Record<string, { sampled: number; defects: number }> = {};
  const compEvidenceById = new Map<string, SampleEvidence>();
  for (const e of v1.samples) compEvidenceById.set(e.id, e);
  for (const c of componentSamples.samples) {
    const s = componentStratum(c);
    if (!stratumAgg[s]) stratumAgg[s] = { sampled: 0, defects: 0 };
    stratumAgg[s].sampled++;
    if (compEvidenceById.get(c.component_id)?.ok === false) stratumAgg[s].defects++;
  }
  const connEvidenceById = new Map<string, SampleEvidence>();
  for (const e of v2.samples) connEvidenceById.set(e.id, e);
  // Merge v3 evidence on top — a connection failing either is one defect for stratum.
  const connDefectIds = new Set<string>();
  for (const e of v2.samples) if (!e.ok) connDefectIds.add(e.id);
  for (const e of v3.samples) if (!e.ok) connDefectIds.add(e.id);
  for (const c of connectionSamples.samples) {
    const s = connectionStratum(c);
    if (!stratumAgg[s]) stratumAgg[s] = { sampled: 0, defects: 0 };
    stratumAgg[s].sampled++;
    if (connDefectIds.has(c.connection_id)) stratumAgg[s].defects++;
  }

  const by_stratum: AuditReport['by_stratum'] = {};
  ...
  const sampled =
    v1.sampledCount + v2.sampledCount + v3.sampledCount + v4.sampledCount + v6.sampledCount;
  const defects =
    v1.defectCount + v2.defectCount + v3.defectCount + v4.defectCount + v5.defectCount + v6.defectCount;
  ...
  if (plan === 'SPRT') {
    ...
    const obs: Array<0 | 1> = [];
    for (const e of v1.samples) obs.push(e.ok ? 0 : 1);
    for (const e of v2.samples) obs.push(e.ok ? 0 : 1);
    for (const e of v3.samples) obs.push(e.ok ? 0 : 1);
    for (const e of v4.samples) obs.push(e.ok ? 0 : 1);
  ...
  const defect_evidence: SampleEvidence[] = [];
  for (const v of verifiers) {
    for (const e of v.samples) {
      if (!e.ok && defect_evidence.length < 20) defect_evidence.push(e);
    }
    if (defect_evidence.length >= 20) break;
  }
...
export function updateEwmaForAudit(
  ...
  for (const [stratum, stats] of Object.entries(report.by_stratum)) {
    ...
    if (result.breach) anyBreach = true;
  }
