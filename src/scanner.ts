...
function getIgnorePatterns(root: string): string[] {
  ...
  if (!fs.existsSync(userFile)) return DEFAULT_IGNORE_PATTERNS;
  try {
...
export function excludeGitIgnoredFiles(root: string, files: string[]): string[] {
  if (files.length === 0 || process.env.NAVGATOR_NO_GIT_IGNORE === '1') return files;
  ...
  if (ignored.size === files.length) return files;
 ...
 */
function endpointNodeName(file: string): string {
  const normalized = file.replace(/\\/g, '/');
  const withoutExtension = normalized.replace(/\.[^.]+$/, '');
  const segments = withoutExtension.split('/').filter(Boolean);
  if (segments[0] === 'src' || segments[0] === 'app' || segments[0] === 'lib') segments.shift();
  if (segments[segments.length - 1] === 'index' && segments.length > 1) segments.pop();
  return segments.join('/') || path.basename(withoutExtension);
}

function endpointNodeLayer(file: string): 'frontend' | 'backend' | 'database' | 'queue' | 'infra' | 'external' {
...
export function resolveFileEndpoints(
  ...
  const compByFile = new Map<string, string>(); // normalized file path → component_id
  for (const comp of components) {
    for (const f of comp.source?.config_files || []) {
      const key = normalizeEndpointPath(f);
      if (!compByFile.has(key)) compByFile.set(key, comp.component_id);
    }
  }
  const fileNodeByPath = new Map<string, string>();
  const resolve = (ref: { component_id: string } | undefined): void => {
    const id = ref?.component_id;
    if (!ref || !id?.startsWith('FILE:')) return;
 ...
 */
function buildEndpointFileComponent(file: string, timestamp: number): ArchitectureComponent {
  return {
 ...
 */
export function discoverStackRoots(
  root: string,
  verbose: boolean
 ...
 */
function computeScanCoverage(
  scannableFiles: string[],
  components: ArchitectureComponent[],
  connections: ArchitectureConnection[]
): { coverage: ScanCoverage | undefined; warnings: ScanWarning[] } {
  ...
  const componentFile = (c: ArchitectureComponent): string | undefined => {
    const cfgFile = c.source?.config_files?.[0];
    if (typeof cfgFile === 'string' && cfgFile.length > 0) return cfgFile;
    const metaFile = (c.metadata as Record<string, unknown> | undefined)?.['file'];
    return typeof metaFile === 'string' ? metaFile : undefined;
  };
  const componentsByLanguage = new Map<string, number>();
  for (const c of components) {
  ...
  const edgesByLanguage = new Map<string, Set<string>>();
  for (const conn of connections) {
    if (conn.connection_type !== 'imports' && conn.connection_type !== 'references') continue;
    const from = conn.from.location?.file ?? conn.code_reference?.file;
    const to = conn.to.location?.file;
    if (!from || !to || from === to) continue;
    const lang = languageOf(from);
    if (!lang) continue;
    if (!edgesByLanguage.has(lang)) edgesByLanguage.set(lang, new Set());
  ...
  const status: ScanCoverage['status'] =
    totalAnalyzedEdges === 0 ? 'none' : hasGap ? 'partial' : 'full';
  ...
  const warnings: ScanWarning[] = blindSpots.map((message) => ({
    type: 'low_confidence',
    message,
  }));
...
export async function scan(
  ...
  const discoveredSourceFiles = await glob(
    '**/*.{ts,tsx,js,jsx,mjs,cjs,py,swift,rs,h,m,go,rb,java,kt,cs,php,scala,c,cpp,cc,hpp}',
    {
      cwd: root,
      ignore: getIgnorePatterns(root),
    }
  ...
  if (!options.quick || options.connections) {
    ...
    try {
      ...
      const pipComponents = allComponents.filter(c =>
        c.source.config_files?.some(f => PIP_MANIFEST_RE.test(f.replace(/\\/g, '/')))
      );
      ...
      // ghost node.
      const PIP_IMPORT_ALIASES: Readonly<Record<string, string>> = {
        beautifulsoup4: 'bs4',
        pillow: 'PIL',
        pyyaml: 'yaml',
      ...
      const canonicalPipName = (name: string) => name.toLowerCase().replace(/[._]+/g, '-');
      const pipKnownPackages: { name: string; component_id: string }[] = [];
      for (const c of pipComponents) {
        ...
        if (alias) pipKnownPackages.push({ name: alias, component_id: c.component_id });
      }
      ...
      const pythonImportResult: ScanResult = pythonSourceFiles.length > 0
        ? await scanImports(root, pythonSourceFiles, pipKnownPackages)
        : { components: [], connections: [], warnings: [] };
      const importResult: ScanResult = {
  ...
  const droppedToSurvivor = new Map<string, string>();
  for (const component of allComponents) {
    const primaryFile = component.source?.config_files?.[0] ?? '';
  ...
  if (droppedToSurvivor.size > 0) {
    for (const conn of allConnections) {
      ...
      if (from) conn.from.component_id = from;
      const to = droppedToSurvivor.get(conn.to?.component_id ?? '');
      if (to) conn.to.component_id = to;
    }
  }
