...
import { loadRegistry, type ProjectEntry } from '../projects.js';
import { readJournal, defaultRegistryDir, type RegistryJournalEvent } from '../registry-journal.js';
 ...
 */
function tmpRootCandidates(): string[] {
  ...
  for (const candidate of candidates) {
    ...
    roots.add(resolved);
    try {
      if (fs.existsSync(resolved)) roots.add(fs.realpathSync(resolved));
    } catch {
 ...
 *      the path text.
 */
export function isTmpRootedPath(targetPath: string): boolean {
...
export function classifyRegistryEntries(entries: ProjectEntry[]): RegistryEntryClassification[] {
...
export function selectPrunableEntries(
  classifications: RegistryEntryClassification[],
...
interface JournalStats {
  ...
  registersInWindow: number;
  registersPerDay: number;
  /** True when the retained window is too short to support a daily rate. */
  insufficientWindow: boolean;
  conflicts: number;
  degradedWrites: number;
}

function computeJournalStats(records: RegistryJournalEvent[]): JournalStats {
...
function buildFindings(report: Omit<HealthReport, 'findings' | 'verdict'>): HealthFinding[] {
  ...
  if (registry.prunable > 0) {
    findings.push({
      ...
      message:
        `${registry.prunable} ${registry.prunable === 1 ? 'entry points' : 'entries point'} at temp ` +
  ...
  if (realMissing > 0) {
    findings.push({
      ...
      message:
        `${realMissing} registered ${realMissing === 1 ? 'project path is' : 'project paths are'} no ` +
  ...
  if (journal.conflicts > 0) {
    findings.push({
      ...
      message:
        `${journal.conflicts} lost-update ${journal.conflicts === 1 ? 'conflict was' : 'conflicts were'} ` +
  ...
  if (journal.degradedWrites > 0) {
    findings.push({
      ...
      message:
        `${journal.degradedWrites} registry ${journal.degradedWrites === 1 ? 'write' : 'writes'} completed ` +
  ...
  if (memory.orphaned > 0) {
    findings.push({
      ...
      message:
        `${memory.orphaned} gator-memory ${memory.orphaned === 1 ? 'record refers' : 'records refer'} to a ` +
  ...
  if (!memory.exists) {
    findings.push({
      ...
      code: 'memory-absent',
      message:
        'No gator-memory store yet at ~/.navgator/memory/ — it is created automatically the first time ' +
  ...
  if (mirror.enabled && !mirror.targetExists) {
    findings.push({
      ...
      message:
        `Mirroring to build-loop-memory is enabled, but the configured target "${mirror.target}" does ` +
...
export async function computeHealth(): Promise<HealthReport> {
  ...
  const base: Omit<HealthReport, 'findings' | 'verdict'> = {
  ...
  const verdict: 'healthy' | 'attention' = findings.some((f) => f.severity === 'warn')
    ? 'attention'
    : 'healthy';
