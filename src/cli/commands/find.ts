 ...
 */
function scoreComponent(c: ArchitectureComponent, query: string): number {
  ...
  let score = 0;
  if (name === q) score += 100;
  else if (name.includes(q)) score += 60 + Math.min(20, q.length);

  for (const qt of qTokens) {
    if (nameTokens.includes(qt)) score += 30;
    if (tags.includes(qt)) score += 10;
  }
  if (purpose.includes(q)) score += 20;
  if (file.includes(q)) score += 8;
...
function buildContext(c: ArchitectureComponent): string {
  const parts: string[] = [];
  if (c.role?.purpose) parts.push(c.role.purpose);
