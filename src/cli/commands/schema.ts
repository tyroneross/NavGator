  ...
  program
    ...
    .action(async (modelName, options) => {
      try {
        ...
        const modelConns = connections.filter(c =>
          c.to.component_id === component.component_id && c.connection_type === 'api-calls-db'
        );

        // Separate reads and writes
        const readers: typeof modelConns = [];
        const writers: typeof modelConns = [];
        const readWriters: typeof modelConns = [];
