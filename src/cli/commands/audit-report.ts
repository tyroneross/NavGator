...
import type { ArchitectureIndex, AuditOracleResult, AuditReport, EwmaStateSnapshot, ProportionInterval } from '../../types.js';
import { runOracles } from '../../audit/oracles/index.js';
import { runSelfTest, type SelfTestReport } from '../../audit/self-test.js';
...
export function registerAuditReportCommand(program: Command): void {
  program
    ...
    .action(async (options: AuditReportOptions) => {
      try {
        ...
        const index = (await loadIndex(config)) as ArchitectureIndex | null;
        if (!index) {
        ...
        if (needGraph) {
          ...
          const componentById = new Map(components.map((c) => [c.component_id, c] as const));
          if (options.oracles || options.scip) {
            data.live_oracles = await runOracles(
              { projectRoot: root, components, connections, componentById, hashes },
              {
                scip: !!options.scip,
          ...
          if (options.selfTest) {
            ...
            data.self_test = await runSelfTest(
              { components, connections },
              { projectRoot: root, config, hashes, K: Math.max(1, parseInt(options.k ?? '10', 10) || 10), plan }
            );
          }
        }
        ...
        console.error('audit-report failed:', (err as Error).message);
...
function ci(i: ProportionInterval | null | undefined): string {
  if (!i) return '';
...
export function renderMarkdown(d: AuditReportData): string {
  const out: string[] = [];
  ...
  if (d.pending_drift_breach) out.push(`\n**Pending drift breach** — next \`--auto\` scan promotes to full + Cochran.`);
  out.push('');

  // ---- Recent audits ----
  out.push(`## Last ${d.audits.length} audits`);
  out.push('');
  if (d.audits.length === 0) out.push('_No audits stored on the timeline yet. Run `navgator scan`._');
  else {
    out.push('| When | Plan | Requested n | Distinct facts | Defects | Rate [95% CI] | Verdict | Breach |');
    out.push('|---|---|---:|---:|---:|---|---|---|');
    for (const a of d.audits) {
      out.push(
        `| ${when(a.timestamp)} | ${a.plan}${a.sprt ? ` (${a.sprt.batches} batch${a.sprt.batches === 1 ? '' : 'es'})` : ''} | ${a.n} | ${a.sampled} | ${a.defects} | ${pct(a.defect_rate)} ${ci(a.ci)} | ${a.verdict} | ${a.drift_breach ? 'yes' : 'no'} |`
      );
    }
  }
  out.push('');
  ...
  if (L) {
    ...
    if (L.screen) {
      ...
      out.push(
        `n=${s.n} per population (components ${s.components_sampled}, connections ${s.connections_sampled}; ${inspected} evaluable units inspected${s.unverifiable ? `, ${s.unverifiable} unverifiable` : ''}), LTPD ${pct(s.ltpd, 0)}, consumer's risk ${pct(s.consumer_risk, 0)}, reject on any defect across both → **${s.defects} defect${s.defects === 1 ? '' : 's'}, ${s.verdict}**.`
      );
      out.push('');
      if (s.defects === 0) out.push(`Zero defects across ${inspected} units licenses: 95% one-sided confidence the true error rate is ≤ ${pct(s.upper_bound_95)}.`);
      out.push(
        `Producer's risk of the combined screen: a map that is truly 99% correct fails it ${pct(s.producers_risk_at_1pct, 0)} of the time` +
      ...
      if (L.sprt) {
        out.push('');
        out.push(
          `SPRT (plan ${L.plan}): ${L.sprt.verdict ?? 'n/a'} after ${L.sprt.batches} batch${L.sprt.batches === 1 ? '' : 'es'}, ${L.sprt.observations}/${L.sprt.cap} observations, log LR ${L.sprt.log_lr.toFixed(2)}. Observations are the stratified, floor-oversampled sample of both populations (biased toward reject); the c=0 screen above is the report verdict.`
        );
      }
      if (L.unverifiable) {
        out.push('');
    ...
    if (L.precision) {
      ...
      out.push('');
      out.push(
        `${p.kind} design (±${pct(p.margin, 0)}): ${p.n_components} components + ${p.n_connections} connections, Neyman allocation with floor n_h ≥ ${p.floor}.` +
    ...
    if (classes.length > 0) {
      ...
      out.push('|---|---:|---:|---:|');
      for (const [k, v] of classes) out.push(`| ${k} | ${v.sampled} | ${v.defects} | ${v.unverifiable ?? 0} |`);
      out.push('');
    }
    ...
    if (L.census) {
      ...
      if (types.length > 0) {
        ...
        out.push('|---|---:|---:|');
        for (const [t, v] of types) out.push(`| ${t} | ${v.bad} | ${v.total} |`);
        out.push('');
      }
      ...
      if (dirs.length > 0) {
        ...
        out.push('|---|---:|---:|');
        for (const [t, v] of dirs) out.push(`| ${t} | ${v.bad} | ${v.total} |`);
        out.push('');
      }
    }
  }
  ...
  out.push('');
  if (!oracles || oracles.length === 0) out.push('_No oracle results. Run `navgator scan` (or `audit-report --oracles`)._');
  else {
    out.push('| Oracle | Stratum | Strength | Truth | Map | TP | FP | FN | Precision [95% CI] | Recall [95% CI] | Frame |');
    out.push('|---|---|---|---:|---:|---:|---:|---:|---|---|---|');
    for (const o of oracles) {
      out.push(
        `| ${o.oracle} | ${o.stratum} | ${o.oracle_strength} | ${o.truth_count} | ${o.map_count} | ${o.tp} | ${o.fp} | ${o.fn} | ${pct(o.precision)} ${ci(o.precision_ci)} | ${pct(o.recall)} ${ci(o.recall_ci)} | ${o.frame_coverage !== undefined ? pct(o.frame_coverage, 0) : ''} |`
      );
    }
    out.push('');
    for (const o of oracles) {
      const bits: string[] = [];
      if (o.fp_samples.length > 0) bits.push(`FP e.g. ${o.fp_samples.slice(0, 5).map((s) => `\`${s}\``).join(', ')}`);
      if (o.fn_samples.length > 0) bits.push(`FN e.g. ${o.fn_samples.slice(0, 5).map((s) => `\`${s}\``).join(', ')}`);
      const notes = o.notes.length > 0 ? o.notes.join('; ') : '';
      if (bits.length > 0 || notes) out.push(`- **${o.oracle}**: ${[...bits, notes].filter(Boolean).join(' — ')}`);
    }
    out.push('');
  }
  ...
  if (charts.length === 0) out.push('_No chart state yet._');
  else {
    ...
    for (const [k, s] of charts.sort((a, b) => a[0].localeCompare(b[0]))) {
      if (!(s.n > 0)) continue; // (a): never render an empty series
      ...
      out.push(
        `| ${k} | ${s.kind ?? 'p'} | ${phase} | ${s.n} | ${centre} | ${last ? `${pct(last.x, 2)} (${last.n})` : ''} | ${last && s.phase === 'frozen' ? `${pct(last.ucl, 2)} / ${pct(last.lcl, 2)}` : ''} | ${last && s.phase === 'frozen' ? `${pct(last.ewma, 2)} (${pct(last.ewma_ucl, 2)}/${pct(last.ewma_lcl, 2)})` : ''} | ${s.cusum ? `${s.cusum.s_hi.toFixed(2)} / ${s.cusum.s_lo.toFixed(2)}` : ''} | ${(s.signals ?? []).join(', ') || (s.breach_pending ? 'legacy-breach' : '')} | ${s.version ?? ''}${s.rebaselined_from ? ` (re-baselined from ${s.rebaselined_from})` : ''} |`
      );
    }
    out.push('');
    ...
    if (withPoints.length > 0) {
      ...
      out.push('');
      for (const [k, s] of withPoints) out.push(`- \`${k}\`: ${s.points.slice(-10).map((p) => (p * 100).toFixed(2)).join(', ')} (pp)`);
      out.push('');
    }
  }
