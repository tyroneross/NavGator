...
export function registerDoctorCommand(program: Command): void {
  program
    ...
    .option(
      '--fix',
      'Prune prunable registry entries and their gator-memory records (destructive — prompts unless --yes)'
    )
    .option('--yes', 'Skip the confirmation prompt for --fix')
    .option(
      '--include-missing',
      'With --fix, also prune registered paths that are missing but not temp-rooted'
    )
...
async function runReport(options: DoctorCommandOptions): Promise<void> {
...
function formatHealthReport(
  report: HealthReport,
  prunableEntries: RegistryEntryClassification[]
): string {
  ...
  lines.push(
    `  Tmp-rooted: ${registry.tmpRooted}   Missing: ${registry.missing}   Prunable: ${registry.prunable}`
  );
  ...
  if (journal.records === 0) {
    ...
    lines.push(
      `  ${journal.registersInWindow} new registration${journal.registersInWindow === 1 ? '' : 's'} ` +
  ...
  if (!memory.exists) {
    ...
    lines.push(
      `  Projects: ${memory.projects}   Orphaned: ${memory.orphaned}   Events: ${memory.events}   Bytes: ${memory.bytes}`
    );
    lines.push(
      `  Last event: ${memory.lastEventAt !== null ? new Date(memory.lastEventAt).toISOString() : 'never'}`
    );
  }
  lines.push('');
  ...
  if (findings.length === 0) {
    ...
    lines.push(
      `Verdict: ${verdict.toUpperCase()} — ${findings.length} finding${findings.length === 1 ? '' : 's'}:`
    );
 ...
 */
function renderSafe(value: string): string {
  // eslint-disable-next-line no-control-regex
  return value.replace(/[\u0000-\u001f\u007f-\u009f]/g, ' ');
}

/** `2026-08-03T09:15:00.000Z` -> `20260803T091500`. */
function compactISOTimestamp(date: Date = new Date()): string {
...
export async function fixRegistry(options: {
  ...
  console.error(
    `The following ${totalCount} ${totalCount === 1 ? 'entry' : 'entries'} will be removed from ` +
  ...
  if (!options.yes) {
    ...
    if (!process.stdin.isTTY) {
      console.error(
        'Refusing to proceed: stdin is not a TTY and --yes was not passed. Re-run with --yes to confirm non-interactively.'
      );
      return { status: 'aborted-non-tty' };
    }

    const confirmed = await confirmPrompt(
      `Remove ${totalCount} ${totalCount === 1 ? 'entry' : 'entries'}? [y/N] `
    );
    if (!confirmed) {
  ...
  const backupPath =
    `${registryPath}.backup-${compactISOTimestamp()}-` +
    `${crypto.randomBytes(3).toString('hex')}`;
  let backupOk = false;
  try {
    fs.copyFileSync(registryPath, backupPath, fs.constants.COPYFILE_EXCL);
    const parsed: unknown = JSON.parse(fs.readFileSync(backupPath, 'utf-8'));
  ...
  for (const entry of removed) {
    ...
    if (removeProjectMemory(entry.path)) removedFromMemory += 1;
  }
  ...
  for (const orphan of orphans) {
    ...
    if (removeProjectMemory(orphan.path)) removedFromMemory += 1;
  }
...
async function runFix(options: DoctorCommandOptions): Promise<void> {
  ...
  console.log(
    `Removed ${outcome.cleanup.removedFromRegistry} entr${outcome.cleanup.removedFromRegistry === 1 ? 'y' : 'ies'} from the registry.`
  );
  console.log(
    `Removed ${outcome.cleanup.removedFromMemory} record${outcome.cleanup.removedFromMemory === 1 ? '' : 's'} from gator-memory.`
  );
}
 ...
 */
export async function describeMirrorRun(): Promise<MirrorRunResult> {
...
async function runMirror(options: DoctorCommandOptions): Promise<void> {
  const outcome = await describeMirrorRun();
  await emitMirrorOutcome(options, outcome);
}

async function emitMirrorOutcome(
  options: DoctorCommandOptions,
  payload: { mirrored: number; skipped: number; ran: boolean; reason?: string }
): Promise<void> {
  if (options.agent) {
