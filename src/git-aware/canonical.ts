 ...
 */
export async function writeSnapshotForCurrentRef(root: string): Promise<WriteSnapshotResult> {
...
async function readSnapshotFile(target: string): Promise<Snapshot | null> {
  try {
...
export async function readCanonicalSnapshot(root: string): Promise<Snapshot | null> {
 ...
 */
export async function readBranchSnapshot(root: string, ref?: string): Promise<Snapshot | null> {
...
export async function pruneBranchSnapshots(
  ...
  for (const entry of entries) {
    if (liveSlugs.has(entry)) continue;
    try {
