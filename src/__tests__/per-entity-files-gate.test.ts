...
function baseConfig(perEntityFiles: boolean): NavGatorConfig {
  return {
    storageMode: 'local',
...
describe('R6 — per-entity files gate', () => {
  ...
  it('migratePerEntityFiles removes legacy *.json files + empty dirs (perEntityFiles=false)', async () => {
    ...
    expect(fs.readFileSync(path.join(storageRoot, 'graph.json'), 'utf-8')).toBe(
      '{"keep":true}'
    );
    expect(fs.readFileSync(path.join(storageRoot, 'connections.jsonl'), 'utf-8')).toBe(
      '{"keep":true}\n'
    );
  });
