...
export function resolveActor(): JournalActor {
  ...
  let actor: JournalActor = 'unknown';
...
function parseRecord(line: string): RegistryJournalEvent | null {
  const trimmed = line.trim();
  if (!trimmed) return null;
  try {
    const parsed = JSON.parse(trimmed) as RegistryJournalEvent;
    if (typeof parsed?.op !== 'string' || typeof parsed?.ts !== 'number') return null;
    return parsed;
  } catch {
...
function matchesFilter(event: RegistryJournalEvent, options: ReadJournalOptions): boolean {
  if (options.conflictsOnly) return event.op === 'conflict';
  if (options.op && event.op !== options.op) return false;
  if (options.actor && event.actor !== options.actor) return false;
  return true;
}

function matchCount(lines: string[], options: ReadJournalOptions): number {
  let count = 0;
  for (const line of lines) {
    const record = parseRecord(line);
    if (record && matchesFilter(record, options)) count++;
  }
  return count;
}
...
export function readJournal(options: ReadJournalOptions = {}): RegistryJournalEvent[] {
  ...
  const filtered: RegistryJournalEvent[] = [];
  for (const line of lines) {
    const record = parseRecord(line);
    if (record && matchesFilter(record, options)) filtered.push(record);
  }
...
export function formatJournal(
  events: RegistryJournalEvent[],
  options: FormatJournalOptions = {}
): string {
  if (events.length === 0) {
  ...
  lines.push(
    `${pad('TIME', 20)}${pad('ACTOR', 11)}${pad('PID', 8)}${pad('OP', 10)}${pad('REV', 7)}${pad('ENTRIES', 9)}DETAIL`
  );
  ...
  for (const e of events) {
    ...
    if (e.digest) detail.push(e.digest);
    if (e.note) detail.push(e.note);

    lines.push(
      `${pad(time, 20)}${pad(e.actor, 11)}${pad(String(e.pid), 8)}${pad(e.op, 10)}${pad(
        String(e.rev),
        7
      )}${pad(String(e.entries), 9)}${detail.join('  ')}`
    );
  }
  ...
  if (conflicts > 0) {
    lines.push(
      `${events.length} entries, ${conflicts} lost-update conflict${
        conflicts === 1 ? '' : 's'
      } detected and merged.`
    );
