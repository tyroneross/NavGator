...
export async function buildReviewReport(
  opts: { projectRoot?: string; component?: string } = {}
  ...
  const runtime_topology: Record<string, number> = {};
  ...
  let llm: ReviewLLMSummary | undefined;
  try {
    let prompts;
    try {
...
export function formatReviewReport(report: ReviewReport): string {
  ...
  if (report.violations.length > 0) {
    ...
    const bySev: Record<string, RuleViolation[]> = {};
  ...
  if (report.focus) {
    ...
    if (report.focus.affected.length > 0) {
      lines.push(
        `Affected: ${report.focus.affected.slice(0, 5).join(", ")}${
          report.focus.affected.length > 5 ? ` +${report.focus.affected.length - 5} more` : ""
        }`
      );
    }
  }
