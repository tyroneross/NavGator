 ...
 */
function resolveScipBin(): string | null {
 ...
 */
export async function runScip(
  projectRoot: string,
  options: { inferTsconfig?: boolean; maxFileBytes?: string; timeoutMs?: number } = {}
): Promise<ScipResult> {
  const t0 = Date.now();
  const result: ScipResult = {
    ok: false,
    edges: [],
    documents_indexed: 0,
    duration_ms: 0,
  ...
  if (options.inferTsconfig) args.push('--infer-tsconfig');
  if (options.maxFileBytes) args.push('--max-file-byte-size', options.maxFileBytes);
  ...
  try {
    ...
    const req = createRequire(import.meta.url);
    const scipMod = req(path.join(path.dirname(bin), 'scip.js')) as { scip: { Index: { deserializeBinary: (b: Uint8Array) => { toObject: () => unknown } } } };
    const buf = fs.readFileSync(outFile);
    const idx = scipMod.scip.Index.deserializeBinary(buf);
    const obj = idx.toObject() as {
      documents: Array<{
        relative_path: string;
        occurrences: Array<{ symbol: string; symbol_roles?: number; range: number[] }>;
        symbols: Array<{ symbol: string; display_name?: string }>;
      }>;
    };
    ...
    const symbolDefDoc = new Map<string, string>();
    const symbolDisplayName = new Map<string, string>();
    for (const doc of obj.documents) {
      for (const s of doc.symbols ?? []) {
        if (!symbolDefDoc.has(s.symbol)) symbolDefDoc.set(s.symbol, doc.relative_path);
        if (s.display_name) symbolDisplayName.set(s.symbol, s.display_name);
      }
      // Definitions (symbol_roles bit 0 = 1) also map symbol → doc.
      for (const occ of doc.occurrences ?? []) {
        if ((occ.symbol_roles ?? 0) & 0x1) {
          if (!symbolDefDoc.has(occ.symbol)) symbolDefDoc.set(occ.symbol, doc.relative_path);
        }
      }
    }
    ...
    result.error = `parse failed: ${(err as Error).message}`;
  } finally {
    try { fs.unlinkSync(outFile); } catch { /* ignore */ }
    result.duration_ms = Date.now() - t0;
  }
 ...
 */
export function crossFileEdges(edges: ScipEdge[]): ScipEdge[] {
