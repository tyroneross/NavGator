 ...
 */
function normalizeToFileId(query: string): string {
 ...
 */
export function looksLikeFilePath(query: string): boolean {
  return (
    query.includes('/') ||
    query.includes('.ts') ||
    query.includes('.js') ||
 ...
 */
export function resolveFileConnections(
  query: string,
  ...
  const importedBy: ArchitectureConnection[] = [];
  const imports: ArchitectureConnection[] = [];
  const otherFrom: ArchitectureConnection[] = [];
  const otherTo: ArchitectureConnection[] = [];
  ...
  for (const conn of allConnections) {
    ...
    if (conn.connection_type === 'imports') {
      if (fromMatch) imports.push(conn);
      if (toMatch) importedBy.push(conn);
    } else {
      if (fromMatch) otherFrom.push(conn);
      if (toMatch) otherTo.push(conn);
    }
  }
 ...
 */
export function formatFileImpact(fc: FileConnections): string {
  const lines: string[] = [];
 ...
 */
export function formatFileConnections(fc: FileConnections): string {
  const lines: string[] = [];
