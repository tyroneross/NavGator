...
import {
  ...
  type DeepMapFinding,
  type DeepMapFindingKind,
  type DeepMapIngestReport,
  type DeepMapPacket,
  type DeepMapRejection,
  type DeepMapRejectionReason,
  type DeepMapTier,
} from './types.js';
import {
  getPacketsPath,
...
/** Descriptor passed to `validateResultPayload` — the trusted facts about the packet a result claims to answer. */
interface PacketDescriptor {
  packet_id: string;
  tier: DeepMapTier;
  run_id: string;
...
const DEEP_MAP_FINDING_KINDS: readonly DeepMapFindingKind[] = [
  'purpose',
  'responsibility',
  'concern',
  'inefficiency',
 ...
 */
function isGroundedEvidence(evidence: string, knownFilePaths: Set<string>): boolean {
  if (knownFilePaths.has(evidence)) return true;
  const colonIdx = evidence.indexOf(':');
  if (colonIdx > 0 && knownFilePaths.has(evidence.slice(0, colonIdx))) return true;
  const hashIdx = evidence.indexOf('#');
  if (hashIdx > 0 && knownFilePaths.has(evidence.slice(0, hashIdx))) return true;
  return false;
}
 ...
 * neither.
 */
function validateOneFinding(
  entry: unknown,
  index: number,
  packet: PacketDescriptor,
  packetComponentIds: Set<string>,
  options: Pick<IngestOptions, 'knownComponentIds' | 'knownFilePaths'>
): { finding: DeepMapFinding } | { rejection: DeepMapRejection } {
  ...
  if (typeof componentId !== 'string' || !options.knownComponentIds.has(componentId)) {
    return rejectFinding(
      packet.packet_id,
      index,
      'unknown_component',
      `component_id ${JSON.stringify(componentId)} was not found in the tier-0 scan`
    );
  }
  ...
  if (packet.tier !== 3 && !packetComponentIds.has(componentId)) {
    return rejectFinding(
      ...
      index,
      'unknown_component',
      `component_id ${componentId} is real but outside packet ${packet.packet_id}'s scope (tier ${packet.tier})`
    );
  }

  const kind = f['kind'];
  if (typeof kind !== 'string' || !DEEP_MAP_FINDING_KINDS.includes(kind as DeepMapFindingKind)) {
    return rejectFinding(
      packet.packet_id,
      index,
      'schema_violation',
      `kind ${JSON.stringify(kind)} is not one of ${DEEP_MAP_FINDING_KINDS.join(', ')}`
    );
  }
  ...
  if (!sanitizedEvidence.some((e) => isGroundedEvidence(e, options.knownFilePaths))) {
    return rejectFinding(
      ...
      index,
      'missing_evidence',
      'no evidence entry references a known repo file path'
    );
  }
  ...
  const finding: DeepMapFinding = {
    ...
    kind: kind as DeepMapFindingKind,
    text,
...
export function validateResultPayload(
  raw: unknown,
  packet: { packet_id: string; tier: DeepMapTier; run_id: string; component_ids: string[] },
  options: Pick<IngestOptions, 'knownComponentIds' | 'knownFilePaths'>
): { findings: DeepMapFinding[]; rejections: DeepMapRejection[] } {
  const findings: DeepMapFinding[] = [];
  const rejections: DeepMapRejection[] = [];
  ...
  const rawFindings = (raw as Record<string, unknown>)['findings'];
  ...
  entries.forEach((entry, index) => {
    ...
    if ('rejection' in result) rejections.push(result.rejection);
 ...
 */
export function ingestRun(options: IngestOptions): IngestResult {
  const persist = options.persist ?? true;
  const { runId, config, projectRoot } = options;

  const packetFiles = listPacketFiles(runId, config, projectRoot);
  const packetsById = new Map<string, DeepMapPacket>();
  ...
  for (const fileName of resultFileNames) {
    ...
    try {
      ...
      const validated = validateResultPayload(
        raw,
        { packet_id: packet.packet_id, tier: packet.tier, run_id: packet.run_id, component_ids: packet.component_ids },
        { knownComponentIds: options.knownComponentIds, knownFilePaths: options.knownFilePaths }
      );
  ...
  const report: DeepMapIngestReport = {
