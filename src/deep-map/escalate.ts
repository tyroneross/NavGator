...
import { selectMappableComponents, type ComponentFilterOptions } from './filter.js';
 ...
 */
export function percentileIndex(values: Map<string, number>): Map<string, number> {
  ...
  sorted.forEach(([, v], i) => {
    if (!firstRank.has(v)) firstRank.set(v, i);
  });
...
export function normalizeFileMap(raw: unknown): Record<string, string> {
  ...
  if (files && typeof files === 'object') return files as Record<string, string>;
...
): { byId: Map<string, string[]>; unresolved: number } {
  const idsByName = new Map<string, string[]>();
  for (const c of components) {
    const list = idsByName.get(c.name);
    if (list) list.push(c.component_id);
    else idsByName.set(c.name, [c.component_id]);
  }
...
export function scoreEscalation(
  ...
  for (const c of internal) {
    groupKey.set(
      c.component_id,
      usingCommunities ? `c${community.get(c.component_id) ?? -1}` : `l${c.role.layer}`
    );
  }

  const llmComponentIds = new Set(
    inputs.components.filter((c) => c.type === 'llm').map((c) => c.component_id)
  );
  ...
  for (const id of internalIds) fileCounts.set(id, 0);
  ...
  const pagerankForInternal = new Map<string, number>();
  ...
  const { byId: violationsById, unresolved } = indexViolationsByComponentId(
    inputs.violations,
    inputs.components
  );
  ...
  const ranked: EscalationScore[] = internal.map((c) => {
    ...
    if (signals.centrality > 0)
      reasons.push(
        `centrality ${(signals.centrality * 100).toFixed(0)}th percentile (pagerank ${raw.pagerank.toFixed(4)})`
      );
    if (signals.bridge > 0)
    ...
    if (calls > 0) reasons.push(`${calls} LLM service-call edge(s)`);
    if (signals.size > 0)
