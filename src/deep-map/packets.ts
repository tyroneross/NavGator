...
import {
  ...
  type DeepMapFinding,
  type DeepMapPacket,
  type DeepMapPacketComponent,
  type DeepMapProvenance,
 ...
 */
export function sanitizeForPrompt(input: string, max: number = NAME_CHARS_CAP): string {
...
function toPacketComponent(
  ...
  if (component.stable_id) out.stable_id = component.stable_id;
  return out;
}

function untrustedPrefix(provenance: DeepMapProvenance): string {
  return provenance.untrusted ? `${UNTRUSTED_SOURCE_NOTE}\n\n` : '';
}

function formatComponentsBlock(components: DeepMapPacketComponent[]): string {
...
function formatEdgesBlock(edges: CompactConnection[], emptyLabel: string): string {
  if (edges.length === 0) return emptyLabel;
  return edges
    .slice(0, EDGES_PER_PACKET_CAP)
    .map(
      (e) =>
        `- ${e.f} -> ${e.t} (${e.ct}) at ${sanitizeForPrompt(e.file ?? '', PATH_CHARS_CAP)}:${sanitizeForPrompt(e.sym ?? '')}`
    )
    .join('\n');
}
...
export function buildTier1Packets(input: BuildPacketsInput): DeepMapPacket[] {
  ...
  return input.partition.groups.map((group, index) => {
    ...
    const packetComponents = group.component_ids
      ...
      .filter((c): c is ArchitectureComponent => Boolean(c))
    ...
    const edges = input.connections
      .filter(
        (c) => groupIdSet.has(c.from.component_id) && groupIdSet.has(c.to.component_id)
      )
      .map(toCompactConnection);

    const prompt = finalizePrompt(
      buildTier1Prompt(group, packetComponents, edges, input.provenance)
    );

    const packet: DeepMapPacket = {
...
function buildTier2Prompt(
  ...
  subject: DeepMapPacketComponent | null,
  widerComponents: DeepMapPacketComponent[],
  widerEdges: CompactConnection[],
  directConnections: ArchitectureConnection[],
  provenance: DeepMapProvenance
): string {
...
export function buildTier2Packets(input: BuildPacketsInput): DeepMapPacket[] {
  if (!input.escalation) return [];
  ...
  return input.escalation.escalated.map((score, index) => {
    ...
    const prompt = finalizePrompt(buildTier2Prompt(
      score,
      subject,
      widerComponents,
 ...
 */
function formatFindingsGrouped(findings: DeepMapFinding[]): string {
...
function buildTier3Prompt(
  input: BuildPacketsInput,
  packetComponents: DeepMapPacketComponent[],
  edges: CompactConnection[],
  escalationTable: EscalationScore[],
  sortedFindings: DeepMapFinding[]
): string {
  const escalatedSuffix = input.escalation
    ? `, ${input.escalation.escalated.length} escalated for tier-2`
    : ' (no escalation data)';
  const escalationBlock =
    escalationTable.length > 0
      ? escalationTable
          .map(
            (s) =>
              `- ${sanitizeForPrompt(s.name)} [${s.component_id}] score=${s.score.toFixed(3)} :: ${
                s.reasons.length > 0 ? sanitizeForPrompt(s.reasons.join('; '), 600) : 'no contributing signals'
              }`
          )
          .join('\n')
      : '(none escalated)';
  ...
  return (
    `${untrustedPrefix(input.provenance)}` +
...
export function buildTier3Packet(input: BuildPacketsInput, findings: DeepMapFinding[]): DeepMapPacket | null {
  ...
  const sortedFindings = [...findings].sort((a, b) => {
    if (a.component_id !== b.component_id) return a.component_id < b.component_id ? -1 : 1;
