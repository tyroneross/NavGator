...
export function buildSnapshotFromRecords(
  components: SnapshotSourceComponent[],
  connections: SnapshotSourceConnection[],
  reason = 'post-scan'
): Snapshot {
  const componentIdToName = new Map<string, string>();
  for (const c of components) {
...
export interface SnapshotSourceComponent {
  ...
  role: { layer: SnapshotComponent['layer']; critical: boolean };
}
...
export interface SnapshotSourceConnection {
  ...
  from: { component_id: string };
  to: { component_id: string };
  connection_type: SnapshotConnection['type'];
  code_reference?: { file?: string };
}
