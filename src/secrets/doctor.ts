 ...
 */
export const ALL_SECRET_CLASSES: SecretClass[] = [
  'aws_key',
  'gcp_key',
  'azure_key',
  'github_token',
 ...
 */
export const HIGH_CONSEQUENCE_CLASSES: SecretClass[] = [
  'private_key',
  'aws_key',
  'gcp_key',
  'azure_key',
...
const SEVERITY_RANK: Record<DiagnosticSeverity, number> = { error: 3, warn: 2, info: 1 };

export interface DoctorInput {
  findings: Array<SecretFinding & { foundBy?: string[] }>;
  engines: EngineCapability[];
  scannedPaths: string[];
}

function severityWorse(a: DiagnosticSeverity | 'none', b: DiagnosticSeverity): DiagnosticSeverity | 'none' {
  if (a === 'none') return b;
  return SEVERITY_RANK[b] > SEVERITY_RANK[a] ? b : a;
}

export function buildDoctorReport(input: DoctorInput): SecretDoctorReport {
  ...
  const specificityBreakdown: Record<DetectorSpecificity, number> = {
    structured: 0,
    heuristic: 0,
    entropy: 0,
  };
  ...
  const availableClassUnion = new Set<SecretClass>();
  for (const e of availableEngines) for (const c of e.classes) availableClassUnion.add(c);
  ...
  const blindSpotGroups = new Map<string, { engineId: string; corpus: SecretLocation['corpus']; paths: Set<string> }>();
  for (const engine of availableEngines) {
    for (const path of scannedPaths) {
      const corpus = classifyCorpus(path);
      if (engine.corpora.includes(corpus)) continue;
  ...
  const singleEngineClasses: SecretClass[] = [];
  const uncoveredClasses: SecretClass[] = [];
  for (const cls of ALL_SECRET_CLASSES) {
    if (cls === 'unknown') continue;
    const coveringEngines = coverageMatrix[cls];
    if (coveringEngines.length === 0) uncoveredClasses.push(cls);
    else if (coveringEngines.length === 1) singleEngineClasses.push(cls);
  }
  ...
  const disagreementCounts = new Map<string, { secretClass: SecretClass; engineId: string; count: number }>();
  for (const finding of findings) {
    const locationCorpora = new Set(finding.locations.map(l => l.corpus));
    const capableEngines = availableEngines.filter(
      e => e.classes.includes(finding.secretClass) && e.corpora.some(c => locationCorpora.has(c))
    );
    ...
    for (const engine of missing) {
      ...
      else disagreementCounts.set(key, { secretClass: finding.secretClass, engineId: engine.id, count: 1 });
    }
  }
  ...
  const disagreementEntries = Array.from(disagreementCounts.values()).sort((a, b) => {
    const classDiff =
      ALL_SECRET_CLASSES.indexOf(a.secretClass) - ALL_SECRET_CLASSES.indexOf(b.secretClass);
  ...
  const detectorCounts = new Map<string, { engine: string; engineDetector: string; count: number }>();
  for (const f of findings) {
    const key = `${f.engine}::${f.engineDetector}`;
    const existing = detectorCounts.get(key);
    if (existing) existing.count += 1;
    else detectorCounts.set(key, { engine: f.engine, engineDetector: f.engineDetector, count: 1 });
  }
  ...
  // ---- 7. unmapped_class ----
  const unmappedByDetector = new Map<string, number>();
  let unmappedTotal = 0;
  for (const f of findings) {
    if (f.secretClass !== 'unknown') continue;
    unmappedTotal += 1;
  ...
  const unverifiedHighConsequence = findings.filter(
    f => f.liveness === 'unverified' && highConsequenceSet.has(f.secretClass)
  );
  if (unverifiedHighConsequence.length > 0) {
    ...
    diagnostics.push({
      ...
      severity: 'warn',
      summary: `${unverifiedHighConsequence.length} high-consequence finding(s) (${affectedClasses.join(
        ', '
      )}) have never had liveness checked — detection proves the string is shaped like a credential, not that it still works.`,
  ...
  let worstSeverity: DiagnosticSeverity | 'none' = 'none';
  for (const d of diagnostics) worstSeverity = severityWorse(worstSeverity, d.severity);
