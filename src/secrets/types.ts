...
export type DiagnosticKind =
  | 'engine_unavailable'      // an engine is not installed; name what coverage is lost
  | 'corpus_blind_spot'       // a scanned path no available engine can read
  | 'single_engine_class'     // only one engine covers a class — no cross-check
  | 'uncovered_class'         // no available engine covers a class at all
  | 'engine_disagreement'     // found by some engines and not others
  | 'detector_degeneracy'     // a detector firing so broadly it is noise, not signal
  | 'unmapped_class'          // an engine class with no SecretClass mapping
  | 'disposition_debt'        // findings nobody has verified or dispositioned
  | 'liveness_unknown';       // findings never liveness-checked
