...
export function sha256Hex12(value: string): string {
...
const BINARY_EXT_RE =
  /\.(png|jpe?g|gif|webp|ico|bmp|svg|pdf|zip|gz|tar|tgz|bz2|7z|rar|woff2?|ttf|eot|otf|mp3|mp4|mov|avi|wasm|so|dll|dylib|exe|bin|class|jar|pyc|db|sqlite3?)$/i;

const LOCK_FILE_RE =
  /(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|Cargo\.lock|Gemfile\.lock|poetry\.lock|composer\.lock)$/;
...
function isScannableFile(filePath: string): boolean {
  ...
  if (BINARY_EXT_RE.test(base) || LOCK_FILE_RE.test(base)) return false;
  try {
...
export function enumerateFiles(roots: string[]): string[] {
  const out: string[] = [];
  ...
  const visit = (entry: string): void => {
    ...
    if (IGNORED_DIRS.has(path.basename(entry))) return;
...
interface NativePatternDef {
  /** Preserved verbatim as `engineDetector`. */
  id: string;
  secretClass: SecretClass;
  specificity: DetectorSpecificity;
  regex: RegExp;
  /** Capture group holding the value; 0 (default) = the whole match. */
  valueGroup?: number;
}
...
const STRUCTURED_PATTERNS: NativePatternDef[] = [
...
const HEURISTIC_PATTERNS: NativePatternDef[] = [
  {
  ...
  },
  {
    ...
    specificity: 'heuristic',
    regex:
      /(?:^|[\s,({])(?:api[_-]?key|api[_-]?secret|secret[_-]?key|access[_-]?token|auth[_-]?token|private[_-]?key|db[_-]?pass(?:word)?|database[_-]?pass(?:word)?|client[_-]?secret|webhook[_-]?secret|signing[_-]?secret|jwt[_-]?secret|session[_-]?secret|encryption[_-]?key)\s*[=:]\s*["']([^"']{8,})["']/gi,
    valueGroup: 1,
  },
  {
...
const NATIVE_PATTERNS_IN_PRIORITY_ORDER: NativePatternDef[] = [...STRUCTURED_PATTERNS, ...HEURISTIC_PATTERNS];

export const NATIVE_CLASSES: SecretClass[] = Array.from(
  new Set<SecretClass>([...NATIVE_PATTERNS_IN_PRIORITY_ORDER.map((p) => p.secretClass), 'high_entropy']),
);
...
function looksHighEntropy(s: string): boolean {
  if (s.length < 24 || s.length > 200) return false;
...
function overlapsAny(spans: Array<[number, number]>, start: number, end: number): boolean {
  return spans.some(([s, e]) => start < e && end > s);
}

function findPatternMatches(line: string, pattern: NativePatternDef): RawMatch[] {
...
function findHighEntropyMatches(line: string, consumed: Array<[number, number]>): RawMatch[] {
  const out: RawMatch[] = [];
  let m: RegExpExecArray | null;
...
function scanFile(filePath: string, byHash: Map<string, SecretFinding>): void {
  let content: string;
  try {
  ...
  if (lines.some((ln) => ln.length > MAX_LINE_LEN)) return;
  ...
  lines.forEach((line, idx) => {
    ...
    for (const match of scanLine(line)) {
      ...
      const location: SecretLocation = { file: abs, line: lineNo, corpus };
      const existing = byHash.get(hash);
      if (existing) {
        const dup = existing.locations.some((l) => l.file === location.file && l.line === location.line);
        if (!dup) existing.locations.push(location);
...
export class NativeEngine implements SecretEngine {
  readonly id = 'native';

  async capability(): Promise<EngineCapability> {
    return {
      id: this.id,
      available: true,
  ...
  async scan(paths: string[]): Promise<SecretFinding[]> {
    const byHash = new Map<string, SecretFinding>();
