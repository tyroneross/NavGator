...
import type { DetectorSpecificity, EngineCapability, SecretClass, SecretEngine, SecretFinding, SecretLocation } from '../types.js';
import { classifyCorpus } from './native.js';

const execFileAsync = promisify(execFile);

/** vault's own `SecretKind` union (secret-patterns.ts), mapped into ours. */
const KIND_MAP: Record<string, SecretClass> = {
...
const VAULT_CLASSES: SecretClass[] = Array.from(new Set(Object.values(KIND_MAP)));

interface VaultTranscriptFinding {
  kind?: string;
  preview?: string;
  valueHash?: string;
  projectDir?: string;
...
interface VaultAuditTranscriptsResult {
  preview?: VaultTranscriptFinding[];
}
...
export class VaultEngine implements SecretEngine {
  readonly id = 'vault';

  constructor(private readonly opts: VaultEngineOptions = {}) {}
  ...
  async capability(): Promise<EngineCapability> {
    const corpora: EngineCapability['corpora'] = ['claude_transcripts'];
  ...
  async scan(paths: string[]): Promise<SecretFinding[]> {
    ...
    if (!requestsRealCorpus) return [];

    const args = ['audit-transcripts', '--dry-run', '--json'];
    if (this.opts.days !== undefined) args.splice(1, 0, '--days', String(this.opts.days));
    ...
    let parsed: VaultAuditTranscriptsResult;
    try {
      parsed = JSON.parse(stdout) as VaultAuditTranscriptsResult;
    } catch {
      return [];
    }
    ...
    for (const f of parsed.preview ?? []) {
      if (!f.kind || !f.valueHash || !f.preview) continue;

      const mappedClass = KIND_MAP[f.kind];
      const secretClass: SecretClass = mappedClass ?? 'unknown';
      const specificity: DetectorSpecificity = !mappedClass ? 'entropy' : f.kind === 'generic' ? 'heuristic' : 'structured';

      const projectDir = f.projectDir ?? 'unknown-project';
      const file = path.join(claudeProjectsRoot, projectDir);
      const location: SecretLocation = { file, corpus: classifyCorpus(file) };
      ...
      if (existing) {
        ...
        if (f.firstSeenAt && (!existing.firstSeen || f.firstSeenAt < existing.firstSeen)) existing.firstSeen = f.firstSeenAt;
        if (f.lastSeenAt && (!existing.lastSeen || f.lastSeenAt > existing.lastSeen)) existing.lastSeen = f.lastSeenAt;
      } else {
