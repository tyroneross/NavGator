...
const DETECTOR_MAP: Record<string, SecretClass> = {
...
const TRUFFLEHOG_CLASSES: SecretClass[] = Array.from(new Set<SecretClass>([...Object.values(DETECTOR_MAP), 'unknown']));
 ...
 */
export function mapDetectorName(detectorName: string): { secretClass: SecretClass; specificity: DetectorSpecificity } {
  const mappedClass = DETECTOR_MAP[detectorName];
  if (mappedClass) return { secretClass: mappedClass, specificity: 'structured' };
...
export class TruffleHogEngine implements SecretEngine {
  readonly id = 'trufflehog';
  private resolved: string | null | undefined; // undefined = not yet resolved, null = confirmed absent

  constructor(private readonly opts: TruffleHogEngineOptions = {}) {}
  ...
  async capability(): Promise<EngineCapability> {
    const corpora: EngineCapability['corpora'] = ['claude_transcripts', 'codex_transcripts', 'repo', 'other'];
  ...
  async scan(paths: string[]): Promise<SecretFinding[]> {
    ...
    for (const line of lines) {
      ...
      let obj: TruffleHogJsonLine;
      try {
        obj = JSON.parse(trimmed) as TruffleHogJsonLine;
      } catch {
        continue;
      }
      ...
      if (!detectorName || !raw || !file) continue;
...
function runAndCollectLines(binary: string, args: string[]): Promise<string[]> {
