...
export async function scanSwiftCode(
  projectRoot: string,
  walkSet?: Set<string>,
  knownPackages?: SwiftKnownPackage[]
  ...
  if (knownPackages && knownPackages.length > 0) {
    ...
    for (const pkg of knownPackages) {
      if (!byModule.has(pkg.module)) byModule.set(pkg.module, pkg.component_id);
    }
  ...
  const gatedLines = new Map<string, boolean[]>();
  for (const f of files) gatedLines.set(f.relativePath, customFlagGatedLines(stripSwiftNonCode(f.content)));
  const gatedDecls = scanFlagGatedDeclarations(files, gatedLines);
  const visibleDeclarations = (file: string, line: number, name: string): Set<string> | undefined => {
    const all = declarations.get(name);
    const gated = gatedDecls.get(name);
    if (!all || !gated || gatedLines.get(file)?.[line - 1]) return all;
  ...
  const componentByName = new Map<string, { id: string; file?: string }>();
  for (const c of components) {
    if (c.type !== 'component' || componentByName.has(c.name)) continue;
    const f = (c.metadata as Record<string, unknown> | undefined)?.['file'];
...
function scanAllImports(files: SwiftFileInfo[]): { module: string; file: string; line: number; snippet: string }[] {
  const results: { module: string; file: string; line: number; snippet: string }[] = [];
  for (const file of files) {
    const seen = new Set<string>();
    for (let i = 0; i < file.lines.length; i++) {
      const line = file.lines[i];
...
function scanMainAttributes(files: SwiftFileInfo[]): { name: string; file: string; line: number }[] {
  const hits: { name: string; file: string; line: number }[] = [];
  for (const file of files) {
    const code = stripSwiftNonCode(file.content);
 ...
 */
function nearestDeclaration(file: string, declFiles: Set<string>): string | undefined {
  if (declFiles.size === 1) return [...declFiles][0];
  if (declFiles.has(file)) return file;
  let best: string | undefined;
  let bestDepth = -1;
  let tied = false;
  for (const d of declFiles) {
    const depth = sharedDirDepth(file, d);
    if (depth > bestDepth) { best = d; bestDepth = depth; tied = false; }
...
function scanTypeDeclarations(files: SwiftFileInfo[]): Map<string, Set<string>> {
 ...
 * ordinary branch.
 */
function customFlagGatedLines(code: string): boolean[] {
  const lines = code.split('\n');
  const out: boolean[] = new Array(lines.length).fill(false);
  const stack: boolean[] = [];
  const isCustomFlag = (cond: string) =>
    /^[A-Za-z_][A-Za-z0-9_]*$/.test(cond) && cond !== 'DEBUG' && cond !== 'SWIFT_PACKAGE';
  for (let i = 0; i < lines.length; i++) {
    ...
    if ((m = /^#if\s+(.+)$/.exec(t))) {
      ...
      if (stack.length) stack[stack.length - 1] = isCustomFlag(m[1].trim());
...
function scanFlagGatedDeclarations(
  files: SwiftFileInfo[],
  gatedLines: Map<string, boolean[]>,
): Map<string, Set<string>> {
  const gated = new Map<string, Set<string>>();
  for (const file of files) {
    const lineGated = gatedLines.get(file.relativePath) ?? [];
    if (!lineGated.some(Boolean)) continue;
    const code = stripSwiftNonCode(file.content);
    const re = /\b(?:struct|class|enum|actor|protocol|typealias)\s+([A-Z]\w*)/g;
    const state = new Map<string, boolean>(); // name → all declarations gated so far
    let m: RegExpExecArray | null;
    while ((m = re.exec(code)) !== null) {
      const line = code.slice(0, m.index).split('\n').length;
...
function scanTypeIdentifiers(file: SwiftFileInfo): { name: string; line: number }[] {
  const code = stripSwiftNonCode(file.content);
  const lines = code.split('\n');
 ...
 */
export function parseSwiftPackageTargets(
  pkgContent: string
): { name: string; type: string; dependencies: string[] }[] {
  const targets: { name: string; type: string; dependencies: string[] }[] = [];
  ...
  while ((decl = declRe.exec(pkgContent)) !== null) {
    ...
    const type =
      kind === 'testTarget' ? 'test' : kind === 'executableTarget' ? 'executable' : 'library';
...
function extractTargetDependencies(targetBody: string): string[] {
  ...
  const bare = /"([^"]+)"/g;
  while ((m = bare.exec(withoutQualified)) !== null) names.push(m[1]);
