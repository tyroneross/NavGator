...
function pathIdentity(file: string): string {
  return normalizeProjectPath(file).replace(/\.md$/i, '');
}

function makeDocument(file: string, content: string, now: number): ParsedDocument {
...
function normalizeMarkdownTarget(target: string): string | null {
  ...
  if (!normalized || NON_DOCUMENT_EXTENSIONS.test(normalized)) return null;
  return normalized;
}

function extractBodyLinks(doc: ParsedDocument): LinkHit[] {
  const hits: LinkHit[] = [];
...
function extractTypedRelationships(doc: ParsedDocument): LinkHit[] {
  const hits: LinkHit[] = [];
...
function addUniqueLookup(map: Map<string, ParsedDocument | null>, key: string, doc: ParsedDocument): void {
  const normalized = key.trim().toLowerCase();
  if (!normalized) return;
  if (!map.has(normalized)) map.set(normalized, doc);
  else if (map.get(normalized)?.file !== doc.file) map.set(normalized, null);
}

function resolveWikiTarget(
  target: string,
  identityLookup: Map<string, ParsedDocument | null>,
  pathLookup: Map<string, ParsedDocument | null>
...
function resolveMarkdownTarget(
  sourceFile: string,
  target: string,
  byFile: Map<string, ParsedDocument>
): ParsedDocument | undefined {
  const normalized = normalizeMarkdownTarget(target);
  if (!normalized) return undefined;
  const relative = path.posix.normalize(
    path.posix.join(path.posix.dirname(sourceFile), normalized.replace(/\\/g, '/'))
  );
...
function makeConnection(source: ParsedDocument, target: ParsedDocument, hit: LinkHit, now: number): ArchitectureConnection {
 ...
 */
export async function scanMarkdownContent(
  projectRoot: string,
  ...
  const identityLookup = new Map<string, ParsedDocument | null>();
  const pathLookup = new Map<string, ParsedDocument | null>();
  for (const doc of documents) {
    if (doc.explicitPageId) addUniqueLookup(identityLookup, doc.pageId, doc);
    for (const alias of doc.aliases) addUniqueLookup(identityLookup, alias, doc);
  ...
  for (const source of documents) {
    if (walkSet && !walkSet.has(source.file)) continue;
