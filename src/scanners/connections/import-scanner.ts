  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (specifier && (specifier.startsWith('.') || isConfiguredAlias(specifier, pathAliases))) {
        const typeOnly =
          (pattern === ES_IMPORT_RE && /^\s*import\s+type\b/.test(match[0])) ||
...
export async function scanImports(
  ...
  for (let i = 0; i < pythonFiles.length; i += batchSize) {
    ...
    for (const result of results) {
      ...
      const emittedPackages = new Set<string>(); // dedupe per-file: one edge per package
