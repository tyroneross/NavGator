...
`npm run clean`, and listed in `.gitignore:46`. The scan indexes it anyway: 46
of its files became components, and they are a Next.js standalone bundle plus
vendored copies of `react`, `react-dom`, and `semver`.

Measured impact on the `transitively-dead` rule after its entry-point fix: 89
components flagged, of which **46 are this tree**. Every one is unreachable in
the honest sense — the bundle is loaded by a spawned process, not by an import —
but none is something an author can act on, so they are noise in a rule that has
just been cleaned up. The same 46 nodes also sit in PageRank, Louvain
communities, and the deep-map escalation ranking: one of them
